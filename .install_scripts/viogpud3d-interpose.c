...
static int load_real(void)
{
    ...
    if (!g_real)
    {
        log_hex64("LoadLibrary(real) FAILED err=", GetLastError());
        return 0;
    }
...
#define DEFINE_STUB(n)                                                                                                 \
    static u64 stub_##n(void)                                                                                          \
    {                                                                                                                  \
        log_hex64("  *** runtime called filled slot=", (u64)g_stub_slot[n]);                                           \
        return 0;                                                                                                      \
    }
...
static long w_retrievesubobject(void *hDevice, u32 subDeviceId, u64 paramSize, void *pParams, u64 outSize, void *pOut)
{
    (void)hDevice;
    (void)paramSize;
    (void)pParams;
...
static void probe_retrieve_subobject(void *pCreateData)
{
    ...
    if (!slot)
    {
        log_str("  <no ppfnRetrieveSubObject>\n");
        return;
    }
...
#define DEFINE_DXGI_STUB(n)                                                                                            \
    static u64 dxgi_stub_##n(void)                                                                                     \
    {                                                                                                                  \
        log_hex64("  *** runtime called filled DXGI slot=", (u64)g_dxgi_stub_slot[n]);                                 \
        return 0x80004001ULL;                                                                                          \
    }
...
static void probe_dxgi_funcs(void *pCreateData)
{
    ...
    if (!tbl)
    {
        log_str("  <no pDXGIDDIBaseFunctions>\n");
        return;
    }
...
static void hook_um_callbacks(void *pCreateData)
{
    ...
    if (!cb)
    {
        log_str("  <no pUMCallbacks>\n");
        return;
    }
    log_hex64("  pUMCallbacks=", (u64)cb);
    for (i = 0; i < UMCALLBACKS_MEMBERS; i++)
    {
        g_umcb_copy[i] = cb[i];
    }
...
static HRESULT w_getversions(void *hAdapter, u32 *puEntries, u64 *pVersions)
{
    ...
    if (hr == 0 && puEntries)
    {
        ...
        {
            u32 i;
            for (i = 0; i < *puEntries && i < 16; i++)
            {
                log_hex64("    version=", pVersions[i]);
            }
        }
    }
    return hr;
...
static void hook_funcs(void *pOpenData, int with_2)
{
    ...
    if (!tbl)
    {
        log_str("  <no adapter funcs table>\n");
        return;
    }
...
__declspec(dllexport) HRESULT OpenAdapter10_2(void *pOpenData)
{
    ...
    if (!load_real() || !g_oa10_2)
    {
        log_str("  no real entry\n");
    ...
    if (hr == 0)
    {
        log_dump("  out:", pOpenData, 64);
        hook_funcs(pOpenData, 1);
    }
    return hr;
}
