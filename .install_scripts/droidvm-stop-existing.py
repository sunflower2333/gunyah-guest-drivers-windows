...
if len(sys.argv) != 2:
    raise SystemExit("usage: droidvm-stop-existing.py <vm-id>")
...
def request(connection: socket.socket, command: str, **fields) -> dict:
    ...
    packet = {"type": "request", "request_id": request_id, "command": command, **fields}
...
with socket.create_connection(("127.0.0.1", port), timeout=10) as daemon:
    ...
    stopped = request(daemon, "vm_stop", vm_id=vm_id)
    print(f"stop={json.dumps(stopped, separators=(',', ':'))}")
