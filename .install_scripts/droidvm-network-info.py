...
with socket.create_connection(("127.0.0.1", port), timeout=10) as daemon:
    ...
    listed = request(daemon, "network_list").get("data", [])
    if not isinstance(listed, list):
        raise RuntimeError("network_list returned non-list data")

    details = []
    for network in listed:
        if not isinstance(network, dict):
            raise RuntimeError("network_list returned a non-object entry")
        network_id = str(network.get("id", ""))
        uuid.UUID(network_id)
        details.append(
            request(daemon, "network_info", network_id=network_id).get("data", {})
        )

print(json.dumps({"networks": listed, "details": details}, separators=(",", ":")))
