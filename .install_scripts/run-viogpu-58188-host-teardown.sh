...
evidence_dir=$1
windows_output=${2:-C:/DroidVM/TurnipRuns/evidence/viogpu-58188-d7fe1b39-host-$(date -u +%Y%m%dT%H%M%SZ)}
adb_bin=${ADB_BIN:-adb}
ssh_bin=${SSH_BIN:-ssh}
...
remote_capture_staged=0

cleanup()
{
    if [ "$remote_capture_staged" -eq 1 ]; then
        "$adb_bin" -s "$adb_serial" shell rm -f "$remote_capture" >/dev/null 2>&1 || true
    fi
}
...
turnip_runner=$script_dir/run-viogpu-58188-turnip.ps1
windows_runner=${WINDOWS_RUNNER:-C:/Users/USER/viogpu-58188-d7fe1b39/run-viogpu-58188-turnip.ps1}
driver_version=${DRIVER_VERSION:-100.6.101.58188}
driver_image_sha256=${DRIVER_IMAGE_SHA256:-b699bb08b055034fe72b1e6676993fb0d0152f663330fa9c34db0820231d40ca}
umd_image_sha256=${UMD_IMAGE_SHA256:-d89f3d930dfe4d0eb3480f3b7495243e7c6805eb3d94d11fc497e7c180345474}
...
expected_turnip_hash=37bfd999a8825019311a8e3aa87ee30cc8fc1a9ab0076ad9b7c5110f112136c3
...
if ! printf '%s\n' "$driver_version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$'; then
    echo "DRIVER_VERSION has an invalid format" >&2
    exit 2
fi
for image_hash in "$driver_image_sha256" "$umd_image_sha256"; do
    if ! printf '%s\n' "$image_hash" | grep -Eq '^[[:xdigit:]]{64}$'; then
        echo "driver image hash has an invalid format" >&2
        exit 2
    fi
done
case "$skip_interactive" in
    0|1) ;;
    *) echo "SKIP_INTERACTIVE must be 0 or 1" >&2; exit 2 ;;
esac
...
set -- powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass \
    -File "$windows_runner" -OutputDirectory "$windows_output" \
    -DriverVersion "$driver_version" \
    -DriverImageSha256 "$driver_image_sha256" \
    -UmdImageSha256 "$umd_image_sha256"
