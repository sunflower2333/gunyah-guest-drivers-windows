[CmdletBinding()]
param(
    [string]$ProbePath = 'C:\Users\user\viogpu-58179-635937d8\kmt\tu_wddm_kmt_probe_arm64.exe',
    [string]$OutputDirectory = 'C:\Users\user\viogpu-58179-635937d8\stress',
...
$expectedProbeHash = '743b17e96f70e1f2dbd7cffad9c0580a4e764b51935dd5f92168cc8539ded814'
$expectedDriverVersion = '100.6.101.58179'
$expectedDriverHash = '02c838b759a76e86f075c238f8dce49d61cd8e857f1f2dde4f55a4a28094c440'
...
function Get-DestroyDiagnostics {
    param([string]$DriverKeyPath)
    $key = Get-Item -LiteralPath $DriverKeyPath
    $values = [ordered]@{}
    foreach ($name in @($key.GetValueNames() | Sort-Object)) {
        if ($name.StartsWith('NativeContextAllocationDestroy', [StringComparison]::Ordinal)) {
            $values[$name] = $key.GetValue(
                $name,
                $null,
                [Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames
...
$destroyDiagnosticsBefore = Get-DestroyDiagnostics $before.DriverKeyPath
foreach ($name in @($diagnosticsBefore.Keys) + @($destroyDiagnosticsBefore.Keys)) {
    Remove-ItemProperty -LiteralPath $before.DriverKeyPath -Name $name -ErrorAction Stop
}

New-Item -ItemType Directory -Path $OutputDirectory | Out-Null
$stamp = Get-Date -Format 'yyyyMMdd-HHmmssfff'
$traceName = "DroidVM-VioGpu-58179-Stress-$stamp"
...
$destroyDiagnosticsAfter = Get-DestroyDiagnostics $after.DriverKeyPath
...
$result = [ordered]@{
    ...
    DestroyDiagnosticsBefore = $destroyDiagnosticsBefore
    DestroyDiagnosticsAfter = $destroyDiagnosticsAfter
    GpuBefore = $before
    GpuAfter = $after
}
