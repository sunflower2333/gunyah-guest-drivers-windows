[CmdletBinding()]
param(
    [string]$ProbePath = 'C:\DroidVM\TurnipRuns\run-33452951065\tu-wddm-vulkan-probe-arm64\tu_wddm_kmt_probe_arm64.exe',
    [string]$OutputDirectory = 'C:\DroidVM\TurnipRuns\evidence\viogpu-58186-c913fd87-negative',
    [int]$NegativeTimeoutMilliseconds = 60000
)
...
$expectedProbeHash = 'ce197d78a2ef8e0ec884505063d242c5adbb28d7705ae474144e060720c31030'
...
function Test-NegativeMarkers {
    ...
    foreach ($marker in @(
        ...
        $matches = [regex]::Matches($Stdout, "(?m)^\s*$([regex]::Escape($marker))\s*$")
        if ($matches.Count -ne 1) {
            $markerErrors.Add("Negative output contains $($matches.Count) exact '$marker' markers, expected 1.")
        }
    }
    if ($Stdout -match '(?m)^\s*Negative submit allocation setup:') {
        $markerErrors.Add('Negative output contains an allocation setup failure marker.')
    }

    $submitCheck = Test-SubmitMarkers $Stdout $Stderr
    foreach ($message in $submitCheck.Errors) {
        $markerErrors.Add($message)
    }

    [pscustomobject]@{
        Passed = $markerErrors.Count -eq 0
        SubmittedFence = $submitCheck.SubmittedFence
        CompletedFence = $submitCheck.CompletedFence
        Errors = @($markerErrors)
    }
}
...
function Invoke-KmtPhase {
    ...
    $markerCheck = Test-NegativeMarkers $stdout $stderr
...
$negative = Invoke-KmtPhase 'negative' @(
    '--stage=allocation',
    '--negative-iova',
    '--negative-submit',
    '--submit-nop'
) $NegativeTimeoutMilliseconds
...
$success = $negative.Success -and $finalCheck.Passed
$summaryPath = Join-Path $OutputDirectory 'negative-summary.json'
$summary = [ordered]@{
    Success = $success
    CompletedAt = (Get-Date).ToString('o')
    ...
    NegativeResultPath = $negative.ResultPath
    NegativeSuccess = $negative.Success
