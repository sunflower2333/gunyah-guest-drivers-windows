...
int wmain(int argc, wchar_t **argv)
{
    ...
    std::printf("ICD ABI pointer_bits=%zu elevated=%lu mode=%ls\n",
                ...
                argc == 4 ? argv[3] : L"real-extension");
    auto proxy_path = std::filesystem::absolute(argv[1]).make_preferred();
    auto proxy = LoadLibraryExW(proxy_path.c_str(),
                                nullptr,
                                LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (!proxy)
    {
        std::printf("FAIL proxy load error=%lu\n", GetLastError());
        return 1;
    }
    ...
    if (argc == 4 && !wcscmp(argv[3], L"missing"))
    {
        ...
        std::puts("PASS missing sibling fails closed");
        return 0;
    }
    if (!backend || !enumerator ||
        enumerator != reinterpret_cast<void *>(GetProcAddress(backend, "clIcdGetPlatformIDsKHR")) ||
    ...
    wchar_t actual[1024];
    if (!GetModuleFileNameW(backend, actual, 1024) || !std::filesystem::equivalent(actual, argv[2]))
    {
        return 1;
    }
    if (argc == 4)
    {
        ...
        auto loader = LoadLibraryExW(loader_path.c_str(),
                                     nullptr,
                                     LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32);
        ...
        if (!enumerate || !info || status || !count || count > 16)
        {
            std::printf("FAIL Khronos loader=%p enumerate=%p info=%p status=%d count=%u error=%lu\n",
                        ...
                        reinterpret_cast<void *>(enumerate),
        ...
        for (cl_uint i = 0; i < count; ++i)
        {
            ...
            if (info(platforms[i], CL_PLATFORM_NAME, sizeof(name), name, nullptr) == CL_SUCCESS &&
                !std::strcmp(name, "VIOGPU dispatch ABI fixture"))
            {
                found = true;
            }
        }
        if (!found)
        {
            std::printf("FAIL fixture absent among %u platforms\n", count);
            return 1;
        }
    }
    std::printf("PASS exact flat ICD backend and %s dispatch ABI pointer_bits=%zu; no GPU execution\n",
                argc == 4 ? "Khronos fixture" : "real runtime extension",
                sizeof(void *) * 8);
    return 0;
}
