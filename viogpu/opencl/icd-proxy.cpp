...
#error Unsupported OpenCL ICD architecture
    ...
    BOOL ok = GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                                 reinterpret_cast<LPCWSTR>(&once),
                                 &self);
    ...
    HMODULE loaded = ok ? LoadLibraryExW(path, nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32)
                        : nullptr;
    if (loaded)
    {
...
{
    if (!InitOnceExecuteOnce(&once, load_backend, nullptr, nullptr))
    {
        return nullptr;
    }
...
{
    auto function = entry<decltype(&clGetExtensionFunctionAddressForPlatform)>("clGetExtensionFunctionAddressForPlatfor"
                                                                               "m");
