...
platform_info(cl_platform_id object, cl_platform_info param, size_t bytes, void *out, size_t *returned)
{
    if (object != reinterpret_cast<cl_platform_id>(&platform))
    {
    ...
    switch (param)
    {
        case CL_PLATFORM_NAME:
            value = "VIOGPU dispatch ABI fixture";
            break;
...
extern "C" cl_int CL_API_CALL clIcdGetPlatformIDsKHR(cl_uint count, cl_platform_id *out, cl_uint *returned)
{
    ...
    if (out)
    {
        *out = reinterpret_cast<cl_platform_id>(&platform);
    }
    return CL_SUCCESS;
}
static void *extension_address(const char *name)
{
    return name && !std::strcmp(name, "clIcdGetPlatformIDsKHR") ? reinterpret_cast<void *>(&clIcdGetPlatformIDsKHR)
                                                                : nullptr;
}
