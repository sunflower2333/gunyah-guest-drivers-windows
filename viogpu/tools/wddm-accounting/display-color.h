...
{
    if (std::isfinite(value)) std::printf("%.6g", static_cast<double>(value));
    ...
    if (status == ERROR_SUCCESS)
    {
        for (const auto &path : paths)
        {
            if (path.targetInfo.adapterId.LowPart != luid.LowPart ||
                path.targetInfo.adapterId.HighPart != luid.HighPart) continue;
            ...
            if (result == ERROR_SUCCESS)
            {
                std::printf(",\"advanced_color_supported\":%u,\"advanced_color_enabled\":%u,"
                            ...
                            info.wideColorEnforced, info.advancedColorForceDisabled,
                            info.bitsPerColorChannel, static_cast<UINT>(info.colorEncoding));
            }
            std::putchar('}');
        }
    }
    ...
    if (SUCCEEDED(factoryResult))
    {
        for (UINT index = 0; index < 64; ++index)
        {
            ...
            const bool matches = SUCCEEDED(adapter->GetDesc1(&description)) &&
                description.AdapterLuid.LowPart == luid.LowPart && description.AdapterLuid.HighPart == luid.HighPart;
            if (matches)
            {
                for (UINT outputIndex = 0; outputIndex < 64; ++outputIndex)
                {
                    ...
                    HRESULT result = output->QueryInterface(__uuidof(IDXGIOutput6), reinterpret_cast<void **>(&output6));
                    DXGI_OUTPUT_DESC1 desc = {};
                    if (SUCCEEDED(result)) result = output6->GetDesc1(&desc);
                    if (!first) std::putchar(',');
                    first = false;
                    std::printf("{\"output_index\":%u,\"hresult\":\"0x%08lx\"", outputIndex, static_cast<ULONG>(result));
                    if (SUCCEEDED(result))
                    {
                        std::printf(",\"device_name\":"); JsonString(desc.DeviceName);
                        std::printf(",\"attached\":%s,\"bits_per_channel\":%u,\"color_space\":%u,\"min_luminance\":",
                                    desc.AttachedToDesktop ? "true" : "false", desc.BitsPerColor, static_cast<UINT>(desc.ColorSpace));
                        JsonFloat(desc.MinLuminance);
                        std::printf(",\"max_luminance\":"); JsonFloat(desc.MaxLuminance);
                        std::printf(",\"max_full_frame_luminance\":"); JsonFloat(desc.MaxFullFrameLuminance);
                    }
                    std::putchar('}');
