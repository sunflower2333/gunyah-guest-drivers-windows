...
{
    ++opens;
    *handle = reinterpret_cast<HANDLE>(uintptr_t(1));
    *info = {};
    return S_OK;
}
...
extern "C" HRESULT WINAPI VioGpuVideoControl(HANDLE, VGPU_VIDEO_CONTROL *c)
{
    if (c->Code == 2)
    {
        VmediaWrite32(c->Payload + 44, VMEDIA_H264);
    }
    if (c->Code == 27)
    {
        VmediaWrite32(c->Payload + 4, 2);
    }
    if (c->Code == 4)
    {
        auto *format = reinterpret_cast<VMEDIA_FORMAT *>(c->Payload);
        VmediaWrite32(format->Data, 64);
        VmediaWrite32(format->Data + 4, 48);
        VmediaWrite32(format->Data + 8, VMEDIA_NV12);
        VmediaWrite32(format->Data + 20, 5760);
        VmediaWrite32(format->Data + 24, 80);
        format->Data[180] = 1;
    }
    if (c->Code == 96)
    {
...
{
    b->BufferBytes = b->Type == VMEDIA_OUTPUT ? 4096 : 5760;
    return S_OK;
}
extern "C" HRESULT WINAPI VioGpuVideoQueue(HANDLE, const VGPU_VIDEO_BUFFER *b, const void *)
{
    if (b->Type == VMEDIA_CAPTURE)
    {
        ++requeues;
        return S_OK;
    }
    VGPU_VIDEO_EVENT e = {};
    e.Event = VMEDIA_EVT_V4L2;
    e.V4l2Type = VMEDIA_EVENT_SOURCE_CHANGE;
    events.push_back(e);
    e = {};
    e.Event = VMEDIA_EVT_DQBUF;
    e.Type = VMEDIA_OUTPUT;
    e.Index = b->Index;
    events.push_back(e);
    e.Type = VMEDIA_CAPTURE;
    e.Index = 0;
    e.BytesUsed = 5760;
    e.TimestampUs = b->TimestampUs;
    events.push_back(e);
    return S_OK;
}
...
{
    CHECK(size >= 5760);
    memset(data, 0x5a, 5760);
    b->BytesUsed = 5760;
    b->TimestampUs = -1234;
    return S_OK;
}
extern "C" HRESULT WINAPI VioGpuVideoStream(HANDLE, VGPU_VIDEO_STREAM *)
{
    return S_OK;
}
static ComPtr<IMFSample> Input()
{
    ...
    BYTE *bytes = nullptr;
    CHECK(SUCCEEDED(buffer->Lock(&bytes, nullptr, nullptr)));
    const BYTE au[] = {0, 0, 0, 1, 0x65};
    memcpy(bytes, au, 5);
    CHECK(SUCCEEDED(buffer->Unlock()));
    CHECK(SUCCEEDED(buffer->SetCurrentLength(5)));
    CHECK(SUCCEEDED(sample->AddBuffer(buffer.Get())));
    CHECK(SUCCEEDED(sample->SetSampleTime(-12340)));
    return sample;
}
int main()
{
    ...
    CHECK(SUCCEEDED(transform->GetAttributes(&attributes)));
    UINT32 async = 0;
    ...
    ComPtr<IMFMediaType> saved;
    CHECK(SUCCEEDED(transform->GetInputCurrentType(0, &saved)));
    UINT32 w = 0, h = 0;
    CHECK(SUCCEEDED(MFGetAttributeSize(saved.Get(), MF_MT_FRAME_SIZE, &w, &h)) && w == 64 && h == 48);
    for (unsigned run = 0; run != 2; ++run)
    {
        ...
        out.pSample = nullptr;
        LONGLONG time = 0;
        CHECK(SUCCEEDED(sample->GetSampleTime(&time)) && time == -12340);
        ComPtr<IMFMediaBuffer> pixels;
        CHECK(SUCCEEDED(sample->ConvertToContiguousBuffer(&pixels)));
        BYTE *data = nullptr;
        DWORD bytes = 0;
        CHECK(SUCCEEDED(pixels->Lock(&data, nullptr, &bytes)) && bytes == 4608);
