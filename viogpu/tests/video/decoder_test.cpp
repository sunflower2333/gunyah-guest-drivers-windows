...
struct Mock final : Transport
{
    ...
    bool Control(uint32_t code, void *payload) override
    {
        ...
        if (code == 2)
        {
            if (VmediaRead32(bytes))
            {
                return false;
            }
            VmediaWrite32(bytes + 44, encoder ? VMEDIA_NV12 : VMEDIA_H264);
        }
        else if (code == 5 && substitute)
        {
            VmediaWrite32(static_cast<VMEDIA_FORMAT *>(payload)->Data + 8, VMEDIA_HEVC);
        }
        else if (code == 4)
        {
            auto &f = *static_cast<VMEDIA_FORMAT *>(payload);
            VmediaWrite32(f.Data, width);
            VmediaWrite32(f.Data + 4, height);
            VmediaWrite32(f.Data + 8, VMEDIA_NV12);
            VmediaWrite32(f.Data + 20, stride * height * 3 / 2);
            VmediaWrite32(f.Data + 24, stride);
            f.Data[180] = 1;
        }
        else if (code == 27)
        {
            VmediaWrite32(bytes + 4, minimum);
        }
        else if (code == 96)
        {
            CHECK(VmediaRead32(bytes) == 1);
            ++drains;
        }
        return true;
    }
    ...
    bool Stream(uint32_t, bool on) override
    {
        if (!on)
        {
            ++stops;
...
int main()
{
    const uint8_t au[] = {0, 0, 0, 1, 0x65};
    {
        ...
        m.Done(VMEDIA_OUTPUT);
        m.Done(VMEDIA_CAPTURE, 0, m.capturedBytes);
        ...
        CHECK(!d.CanInput());
        VGPU_VIDEO_EVENT eos = {};
        eos.Event = VMEDIA_EVT_V4L2;
        eos.V4l2Type = VMEDIA_EVENT_EOS;
    ...
    }
    {
        ...
        Ready(m, d);
        m.Source();
        m.Done(VMEDIA_CAPTURE, 0, 0, VMEDIA_FLAG_LAST);
        m.width = 96;
        m.height = 64;
        m.stride = 112;
    ...
    }
    {
        ...
        m.minimum = 33;
        DecoderSession d(m);
        CHECK(d.Open(0, 64, 48) == Result::Ok);
        m.Source();
