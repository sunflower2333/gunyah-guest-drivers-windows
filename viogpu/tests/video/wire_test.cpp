...
SAME(VMEDIA_BUFFER,v4l2_buffer); SAME(VMEDIA_PLANE,v4l2_plane);
SAME(VMEDIA_FORMAT,v4l2_format); SAME(VMEDIA_REQBUFS,v4l2_requestbuffers);
...
int main()
{
    ...
    VMEDIA_IOCTL cmd; VMEDIA_BUFFER b; VMEDIA_PLANE p; VMEDIA_SG sg;
    memcpy(&cmd,packet,16); memcpy(&b,packet+16,88); memcpy(&p,packet+104,64); memcpy(&sg,packet+168,16);
    ...
    for (uint32_t r:p.Reserved) assert(r==0);
    int64_t us=0; assert(VmediaTimeToUs(b.Timestamp,&us) && us==-1);
    ...
    VMEDIA_TIMEVAL bad={INT64_MAX,0}; assert(!VmediaTimeToUs(bad,&us));
    bad.Seconds=0; bad.Microseconds=1000000; assert(!VmediaTimeToUs(bad,&us));
...
#endif
    VMEDIA_FORMAT f={}; f.Type=VMEDIA_CAPTURE; f.Data[180]=1;
    VmediaWrite32(f.Data+20,3110400);
    uint32_t size=0; assert(VmediaFormatSize(&f,1<<24,&size) && size==3110400);
    f.Data[180]=2; assert(!VmediaFormatSize(&f,1<<24,&size));
