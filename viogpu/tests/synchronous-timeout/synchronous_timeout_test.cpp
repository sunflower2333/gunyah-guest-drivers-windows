...
UCHAR __ImageBase;
static void *testReturnAddress()
{
    return reinterpret_cast<void *>(reinterpret_cast<ULONG_PTR>(&__ImageBase) + 0x4321);
}
...
{
    static_cast<KEVENT *>(ctx)->signaled = true;
}
...
struct CtrlQueue
{
    ...
    {
        ++queued;
        check(buf->complete_cb == NotifyEventCompleteCB && buf->complete_ctx == &buf->completion_event && !buf->auto_release &&
                                                                                                                  !buf->completion_event.signaled,
              "queue owns explicit completion buffer");
        return queueResult;
    }
...
static void lifecycle()
{
    ...
    check(!buf.complete_cb && !buf.complete_ctx && !buf.synchronous_epoch_state && !queue.GetFirstSynchronousTimeout(&diagnostic),
          "rejected descriptor cleans callback without timeout");
    queue.queueResult = 0;
    ...
    check(!queue.SubmitSynchronousLocked(&buf,
                                         &release,
                                         &submitted) && !release && submitted && !queue.GetFirstSynchronousTimeout(&diagnostic),
    ...
    check(queue.GetFirstSynchronousTimeout(&diagnostic) && diagnostic.Flags == 3 && diagnostic.Type == VIRTIO_GPU_CMD_RESOURCE_UNREF &&
                                                                                                              ...
                                                                                                              diagnostic.WaitStatus == STATUS_TIMEOUT &&
                                                                                                              diagnostic.CallerRva == 0x4321 &&
                                                                                                              diagnostic.CommandBytes == sizeof(command) &&
                                                                                                              diagnostic.EpochGeneration == 1,
          "timeout captures command and prepoison submission epoch");
...
template <typename Wire> static void resource(ULONG type)
{
    Wire command = {};
    ...
    for (size_t size = 0; size <= sizeof(Wire); ++size)
    {
        ...
        check(diagnostic.Flags == (size < sizeof(GPU_CTRL_HDR) ? 0U
                                   : size < sizeof(Wire)       ? 1U
                                                               : 3U),
...
static void decoding()
{
    ...
    buf.size = sizeof(command);
    for (ULONG type :
         {0xffffU, static_cast<ULONG>(VIRTIO_GPU_CMD_SUBMIT_3D), static_cast<ULONG>(VIRTIO_GPU_CMD_CTX_DESTROY)})
    {
        command.hdr.type = type;
        VIOGPU_SYNCHRONOUS_TIMEOUT_DIAGNOSTIC diagnostic = {};
        VioGpuDecodeSynchronousTimeoutCommand(&buf, &diagnostic);
...
static void publication()
{
    ...
    std::thread reader([&] {
        ...
        while (!done.load())
        {
            ...
            {
                if (read.Flags != 3 || read.ResourceId != 19 || read.WaitStatus != STATUS_TIMEOUT ||
                    read.EpochGeneration != 8)
                {
                    consistent = false;
                }
                ++observations;
    ...
    queue.RecordFirstSynchronousTimeout(&buf,
                                        STATUS_TIMEOUT,
                                        VioGpuMakeSynchronousEpochState(8, VioGpuSynchronousEnabled),
                                        0);
    for (int i = 0; i < 1000; ++i)
    {
        queue.RecordFirstSynchronousTimeout(&buf, -1, VioGpuMakeSynchronousEpochState(9, VioGpuSynchronousEnabled), 0);
    }
    while (observations.load() < 100)
    {
        std::this_thread::yield();
    }
    done = true;
