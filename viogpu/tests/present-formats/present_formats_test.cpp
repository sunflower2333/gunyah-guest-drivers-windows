...
static bool color_copy(D3DDDIFORMAT sourceFormat, D3DDDIFORMAT destinationFormat)
{
    ...
    if (!ValidatePresentGeometry(&s, &d, &tx.SourceRect, &tx.DestinationRect, clips, 2))
    {
        return false;
    }
    ExecuteCopy(&tx);
    ...
    for (UINT y = 0; y < 6; ++y)
    {
        for (UINT offset = 0; offset < 37; ++offset)
        {
            ...
            if (lane == 3 && destinationFormat == D3DDDIFMT_X8R8G8B8)
            {
                continue;
            }
            UINT component = destinationFormat == D3DDDIFMT_A8B8G8R8 || lane == 1 || lane == 3 ? lane : 2 - lane;
            if (dst[y * 37 + offset] != rgba[component])
            {
                return false;
            }
        }
...
static bool ten_bit_copy(D3DDDIFORMAT format)
{
    ...
    for (UINT y = 0; y < 4; ++y)
    {
        for (UINT x = 0; x < 6; ++x)
        {
            const DWORD word = (x * 97 + 513) | ((y * 131 + 3) << 10) | ((1023 - x - y) << 20) | (((x + y) & 3U) << 30);
...
int main()
{
    ...
    source.Format = D3DDDIFMT_UNKNOWN;
    check(!valid(), "unknown source format");
    source.Format = D3DDDIFMT_A8R8G8B8;
    destination.Format = D3DDDIFMT_UNKNOWN;
    check(!valid(), "unknown destination format");
    destination.Format = D3DDDIFMT_A8R8G8B8;
    source.Pitch = 15;
    check(!valid(), "short source row");
    source.Pitch = 16;
    destination.Pitch = 15;
    check(!valid(), "short destination row");
    destination.Pitch = 16;
    source.BackingSize = 63;
    check(!valid(), "short source backing");
    source.BackingSize = 64;
    destination.BackingSize = 63;
    check(!valid(), "short destination backing");
    destination.BackingSize = 64;
    source.Width = UINT32_MAX;
    check(!valid(), "source width arithmetic");
    source.Width = 4;
    destination.Width = UINT32_MAX;
    check(!valid(), "destination width arithmetic");
    destination.Width = 4;
    clipped.left = -1;
    check(!valid(), "negative clip");
    clipped.left = 1;
    clipped.right = 5;
    check(!valid(), "clip past backing");
    clipped.right = 3;
