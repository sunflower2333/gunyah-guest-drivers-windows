...
struct LIST_ENTRY { LIST_ENTRY *Flink,*Blink; }; using PLIST_ENTRY=LIST_ENTRY*;
...
void RemoveEntryList(PLIST_ENTRY e) { assert(e->Blink->Flink==e && e->Flink->Blink==e);e->Blink->Flink=e->Flink;e->Flink->Blink=e->Blink; }
...
void KeAcquireSpinLockAtDpcLevel(KSPIN_LOCK *m) { m->lock(); }
void KeReleaseSpinLockFromDpcLevel(KSPIN_LOCK *m) { m->unlock(); }
KIRQL KeGetCurrentIrql() { return irql; }
struct KEVENT { bool set; };
void KeInitializeEvent(KEVENT *e,int,bool set) { e->set=set; }
void RtlZeroMemory(void *p,size_t n) { memset(p,0,n); }
std::unordered_set<void*> allocations;
void *ExAllocatePoolUninitialized(int,size_t n,unsigned) { void *p=malloc(n);assert(p);assert(allocations.insert(p).second);return p; }
void ExFreePoolWithTag(void *p,unsigned) { assert(allocations.erase(p)==1);free(p); }
// INSERT_RECORDS
...
VIOGPU_VBUFFER_TERMINAL_CLAIM VioGpuClaimVbufferTerminalCallbacks(PGPU_VBUFFER buffer)
{
    ...
    if(callbackDetachedFree) static_cast<VioGpuBuf*>(buffer->pool_owner)->FreeBuf(buffer);
...
{
    p->complete_cb=nullptr;p->complete_ctx=nullptr;p->cancel_cb=nullptr;p->cancel_ctx=nullptr;
    p->queue_error_cb=nullptr;p->queue_error_ctx=nullptr;
}
void VioGpuCompleteVbufferTerminalCallbacks(PGPU_VBUFFER) {}
BOOLEAN VioGpuWaitForVbufferTerminalCallbacks(PGPU_VBUFFER p) { return p!=held; }
void cancelled(void*) { ++cancellations; }
// INSERT_PRODUCTION
...
void verify(VioGpuBuf &pool,UINT inUse,UINT cached)
{
    auto count=[&](LIST_ENTRY *h,bool live) {
        ...
        for(auto e=h->Flink;e!=h;e=e->Flink) {
            ...
            if(live) assert(p->pool_owner==&pool && p->pool_in_use);
            else assert(!p->pool_in_use);
...
{
    {
        ...
        assert(pool.Init(8)); verify(pool,0,8);
        assert(!pool.GetBuf(0,8,nullptr)); assert(!pool.GetBuf(8,25,nullptr));
        std::vector<PGPU_VBUFFER> buffers;
        for(UINT i=0;i<2048;++i) buffers.push_back(pool.GetBuf(8,8,nullptr));
        verify(pool,2048,0);
        other.FreeBuf(buffers[10]); verify(pool,2048,0); // no foreign-list mutation
        std::mt19937 random(91);std::shuffle(buffers.begin(),buffers.end(),random);
        for(auto p:buffers) pool.FreeBuf(p);
        verify(pool,0,8);
        auto p=pool.GetBuf(8,8,nullptr);pool.FreeBuf(p);pool.FreeBuf(p); // still-cached duplicate
        verify(pool,0,8);
        p=pool.GetBuf(8,32,static_cast<char*>(pool.AllocateMemory(32)));
        p->data_buf=pool.AllocateMemoryUninitialized(4096);p->data_size=4096;
        pool.FreeBuf(p);verify(pool,0,8);
        assert(pool.Close());verify(pool,0,0);
    }
    assert(allocations.empty());
    for(bool close : {false,true}) {
        VioGpuBuf pool;assert(pool.Init(2));
        for(UINT i=0;i<17;++i) {
            auto p=pool.GetBuf(8,8,nullptr);p->cancel_cb=cancelled;
            p->data_buf=pool.AllocateMemoryUninitialized(128);p->data_size=128;
            if(i==3) held=p;
        }
        callbackDetachedFree=true;cancellations=0;
        if(close) assert(!pool.Close());else pool.ReclaimBuffers();
        assert(cancellations==16);
        verify(pool,1,close?0:1);
        // Terminal claimant timed out: owner flag must be restored for a later DPC.
        auto pending=held;held=nullptr;callbackDetachedFree=false;
        pool.FreeBuf(pending);verify(pool,0,close?0:2);
        assert(pool.Close());
    }
    assert(allocations.empty());
    puts("PASS production VBUFFER pool: 2048 unordered frees, reuse, ownership, detached reset callbacks, timeout reinsert, external payload cleanup");
}
