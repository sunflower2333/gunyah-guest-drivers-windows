...
LONG InterlockedExchange(volatile LONG *p, LONG n)
{
    LONG o = *p;
    *p = n;
    return o;
}
LONG InterlockedCompareExchange(volatile LONG *p, LONG n, LONG e)
{
    LONG o = *p;
    if (o == e)
    {
...
void KeClearEvent(KEVENT *p)
{
    p->set = false;
}
void KeSetEvent(KEVENT *p, int, bool)
{
    p->set = true;
}
...
bool ExAcquireRundownProtection(RUNDOWN *p)
{
    if (!p->allow)
    {
        return false;
...
void ExReleaseRundownProtection(RUNDOWN *p)
{
...
void ExQueueWorkItem(WORK_QUEUE_ITEM *p, int)
{
    assert(std::find(tasks.begin(), tasks.end(), p) == tasks.end());
    tasks.push_back(p);
}
unsigned reportCount = 0;
template <class... T> void DbgPrintEx(int, int, const char *, T...)
{
    ++reportCount;
}
// INSERT_RECORDS
...
struct VioGpuDod
{
    ...
    ~VioGpuDod()
    {
        assert(NativePassiveIdleLocked());
        assert(m_HardwareOperations.references == 0);
    }
    BOOLEAN IsHardwareResetRequested()
    {
        return reset;
    }
    BOOLEAN RecordNativeSubmissionFence(UINT fence)
    {
        if (fence <= lastFence)
        {
            return false;
    ...
    bool AcquireNativeSubmissionOperation()
    {
        return !reset;
    }
    ...
    void NotifyNativeSubmissionFault(UINT, NTSTATUS, UINT, UINT, bool)
    {
        ++faults;
        reset = true;
    }
...
void DereferenceRenderSubmission(VIOGPU_WDDM_SUBMISSION *s)
{
    assert(s->refs > 0);
    if (--s->refs == 0)
    {
        assert(live.erase(s) == 1);
        delete s;
    }
}
void ReleaseRenderWorkReference(VIOGPU_WDDM_SUBMISSION *s)
{
    if (InterlockedCompareExchange(&s->WorkReferenceHeld, 0, 1) == 1)
    {
...
void QuarantineSubmission(VIOGPU_WDDM_SUBMISSION *s, int, bool)
{
    if (s->registry)
    {
        s->registry = false;
        s->VirtioBuffer = nullptr;
        DereferenceRenderSubmission(s);
    }
}
NTSTATUS ValidateNativeRenderBindings(VIOGPU_WDDM_SUBMISSION *)
{
    return 0;
}
...
void barrier(PVOID p)
{
    ...
    s->Adapter->barriers.push_back(s->FenceId);
    terminal(s);
}
VIOGPU_WDDM_SUBMISSION *submit(VioGpuDod &a, UINT fence, bool render = true)
{
    auto s = new VIOGPU_WDDM_SUBMISSION{};
    live.insert(s);
    s->Adapter = &a;
    s->FenceId = fence;
    InitializeListHead(&s->Work.Link);
    s->Work.Context = s;
    s->Work.CancelRequested = &s->CancelRequested;
    s->Work.Routine = render ? NativeRenderDispatchWorker : barrier;
    s->Work.CancelRoutine = NativeRenderDispatchCancelled;
    s->Work.PipelineEligible = render;
    s->Work.PayloadBytes = 128;
    s->Work.AllocationReferences = 3;
    assert(a.QueueNativePassiveWork(&s->Work, fence));
    return s;
}
...
{
    {
        ...
        auto reports = reportCount;
        a.ReportNativeSubmitPerf();
        a.ReportNativeSubmitPerf();
        assert(reportCount == reports + 1);
    }
    {
        VioGpuDod a;
        auto first = submit(a, 1);
        submit(a, 2, false);
        submit(a, 3);
        pump();
        ...
        terminal(first);
        pump();
        assert(a.barriers == std::vector<UINT>{2});
        assert(a.issued.size() == 2);
        finish(a);
    }
    {
        VioGpuDod a;
        submit(a, 1);
        auto blocked = submit(a, 2, false);
        submit(a, 3);
        pump();
        assert(a.CancelNativePassiveWork(&blocked->Work) == VioGpuNativePassiveWorkRemoved);
        NativeRenderDispatchCancelled(blocked);
        pump();
        assert(a.issued.size() == (VIOGPU_NATIVE_PIPELINE_WINDOW == 1 ? 1U : 2U));
        finish(a);
    ...
    }
    {
        ...
        pump();
        assert(a.issued.size() == 512 && live.empty());
        assert(a.m_NativeSubmitPerf.Retired == 512);
    }
    {
        VioGpuDod a;
        a.failQueue = true;
        submit(a, 1);
        pump();
        assert(a.faults == 1 && live.empty());
        assert(a.m_NativeSubmitPerf.Retired == 1);
    }
    {
        VioGpuDod a;
        for (UINT i = 1; i <= VIOGPU_NATIVE_PIPELINE_WINDOW + 2; ++i)
        {
            submit(a, i);
        }
        pump();
        a.reset = true;
        a.CloseNativePassiveQueue();
        assert(a.m_NativeSubmitPerf.CancelledQueued == 2 && !a.NativePassiveIdleLocked());
        for (auto p : live)
        {
            assert(p->CancelRequested == 1);
        }
        finish(a);
        assert(a.issued.size() == VIOGPU_NATIVE_PIPELINE_WINDOW);
    }
    for (UINT seed = 1; seed <= 30; ++seed)
    {
        ...
        finish(a);
        assert(a.m_NativeSubmitPerf.Accepted == 200);
        assert(a.m_NativeSubmitPerf.Retired == 200);
        assert(a.m_NativeSubmitPerf.HostPendingPeak <= VIOGPU_NATIVE_PIPELINE_WINDOW);
    }
    assert(tasks.empty() && live.empty());
    std::printf("PASS production pipeline window=%u: bounds, out-of-order retirement, barriers, cancellation, fast "
                "callback, enqueue failure, reset, 30 randomized schedules\n",
