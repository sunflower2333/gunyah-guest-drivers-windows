...
enum VIOGPU_HOST_CONTEXT_RESULT
{
...
static bool VioGpuNativeControlFaultsClear(VioGpuAdapter *, VIOGPU_NATIVE_CONTEXT_OWNER *)
{
    return !faults;
}
class VioGpuAdapter
{
  public:
    ...
    bool IsNativeContextGenerationCurrent(int, ULONGLONG)
    {
        return current;
    }
    VIOGPU_HOST_CONTEXT_RESULT QueryNativeContextParameterLocked(VIOGPU_NATIVE_CONTEXT_OWNER *,
                                                                 int param,
                                                                 PULONGLONG ticks,
                                                                 LONG *hostError)
    {
        if (param != MSM_PARAM_TIMESTAMP)
        {
            failures++;
        }
        *ticks = sample;
