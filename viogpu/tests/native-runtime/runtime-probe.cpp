...
static void checked(HRESULT hr, const char* stage) { require(SUCCEEDED(hr), stage, hr); }
...
static HWND makeWindow() {
    ...
    HWND window = CreateWindowExW(0, wc.lpszClassName, L"VIOGPU Microsoft D3D11 runtime test",
        WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 320, 320,
...
static void d3d11(IDXGIFactory4* factory, IDXGIAdapter1* adapter, bool warp,
    const std::wstring& expectedUmd, const std::wstring& expectedHash) {
    ...
    checked(create(adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT,
        levels, ARRAYSIZE(levels), D3D11_SDK_VERSION, &device, &actual, &context), "D3D11CreateDevice");
    std::printf("D3D11_FEATURE_LEVEL=0x%04x\n", static_cast<unsigned>(actual));
    if (!warp) verifyUmd(expectedUmd, expectedHash);
    ...
    checked(compile(shader, sizeof(shader) - 1, "runtime-probe", nullptr, nullptr,
        "vs", "vs_4_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &vsCode, &errors), "compile-vs");
    errors.Reset();
    checked(compile(shader, sizeof(shader) - 1, "runtime-probe", nullptr, nullptr,
        "ps", "ps_4_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &psCode, &errors), "compile-ps");
    ...
    for (UINT frame = 0; frame < 4; ++frame) {
        ...
        const BYTE expected[4] = {static_cast<BYTE>(frame & 1 ? 0 : 255), static_cast<BYTE>(frame & 1 ? 255 : 0), 0, 255};
        ...
        UINT mismatches = 0;
        if (!mapped.pData || mapped.RowPitch < kSize * 4) mismatches = kSize * kSize;
        else for (UINT y = 0; y < kSize; ++y) for (UINT x = 0; x < kSize; ++x) {
...
static void d3d12(IDXGIAdapter1* adapter, bool warp,
    const std::wstring& expectedUmd, const std::wstring& expectedHash) {
...
int wmain(int argc, wchar_t** argv) {
    ...
    try {
        for (int i = 1; i < argc; ++i) {
            ...
            if (option == L"--self-test-warp") warp = true;
            else if (option == L"--api" && i + 1 < argc) api = argv[++i];
            else if (option == L"--expect-umd" && i + 1 < argc) expectedUmd = argv[++i];
            else if (option == L"--expect-umd-sha256" && i + 1 < argc) expectedHash = argv[++i];
            else require(false, "arguments");
        }
        require(api == L"d3d11" || api == L"d3d12", "api-required");
        if (warp) require(expectedUmd.empty() && expectedHash.empty(), "self-test-not-hardware-acceptance");
        else {
            require(expectedUmd.size() > 3 && expectedUmd[1] == L':' && expectedUmd[2] == L'\\'
                && expectedHash.size() == 64, "expected-umd-identity-required");
            require(_wcsicmp(fileHash(expectedUmd).c_str(), expectedHash.c_str()) == 0,
                "expected-umd-preflight-hash");
        ...
        if (api == L"d3d11") d3d11(factory.Get(), adapter.Get(), warp, expectedUmd, expectedHash);
        else d3d12(adapter.Get(), warp, expectedUmd, expectedHash);
        return 0;
    } catch (const Failure&) { return 1; }
    catch (...) { std::fputs("FAIL stage=unexpected-exception\n", stderr); return 2; }
}
