...
struct DXGKARG_SETVIDPNSOURCEVISIBILITY
{
    ULONG VidPnSourceId;
    BOOLEAN Visible;
};
...
struct VioGpuDod
{
    ...
    unsigned fences = 0;
    void RequestWddmSubmissionDrainAtAnyIrql()
    {
        drained = true;
        opened = false;
    }
    bool WaitForWddmSubmissionDrain()
    {
        return drainSucceeds;
    }
    bool WaitForNativePassiveQueueIdle()
    {
        return idleSucceeds;
    }
    bool OpenNativePassiveQueue()
    {
        return openSucceeds;
    }
    bool OpenWddmPresentTransactions()
    {
        opened = openSucceeds;
        return openSucceeds;
    }
...
{
    ++faultNotifications;
    owner->RequestHardwareResetAtAnyIrql();
}
...
int main()
{
    ...
    for (auto previousPower : {PowerDeviceD0, PowerDeviceD3})
    {
        ...
        check(device.drained && device.opened && device.fences == 1 && device.m_HardwareResetState == VioGpuHardwareActive,
              "publish only completed recovery");
    }
    for (unsigned failure = 0; failure < 7; ++failure)
    {
        ...
        if (failure == 0)
        {
            device.drainSucceeds = false;
        }
        if (failure == 1)
        {
            device.adapter.closeStatus = TeardownFailure;
        }
        if (failure == 2)
        {
            device.adapter.startStatus = STATUS_DEVICE_NOT_READY;
        }
        if (failure == 3)
        {
            device.adapter.resetDuringClose = true;
        }
        if (failure == 4)
        {
            device.adapter.resetDuringStart = true;
        }
        if (failure == 5)
        {
            device.idleSucceeds = false;
        }
        if (failure == 6)
        {
            device.openSucceeds = false;
        }
        ...
        if (failure < 2)
        {
            check(!device.adapter.interruptsAtStart, "failed teardown cannot start new transport");
        }
    }
    VioGpuDod active;
    active.m_HardwareResetState = VioGpuHardwareActive;
    active.m_AdapterPowerState = PowerDeviceD0;
    check(restart(active) == STATUS_SUCCESS && active.fences == 0 && active.adapter.transitions == std::vector<DEVICE_POWER_STATE>{PowerDeviceD0},
          "ordinary active D0 is preserved");
    VioGpuDod competing;
    ...
    check(post.SetPowerState(0,
                             PowerDeviceD0,
                             PowerActionNone,
                             TRUE) == TeardownFailure && post.adapter.transitions.empty(),
          "ordinary OS D0 still honors POST failure");
