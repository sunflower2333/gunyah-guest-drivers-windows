...
{
    ++checks;
    if (!ok) { std::fprintf(stderr, "FAIL WDK: %s\n", message); std::exit(1); }
}
int main()
{
    VioGpuDod adapter;
    ...
    output.NbSegment = 0;
    auto expected = output; expected.NbSegment = 1;
    DXGKARG_QUERYADAPTERINFO query = {};
    query.pInputData = &input; query.InputDataSize = sizeof(input);
    query.pOutputData = &output; query.OutputDataSize = sizeof(output);
    ...
    storage.fill(0xa5);
    output.pSegmentDescriptor = storage.data(); output.SegmentDescriptorStride = static_cast<UINT>(storage.size());
    ...
    check(descriptor.Size == VIOGPU_WDDM_APERTURE_SIZE && descriptor.Flags.CpuVisible && descriptor.Flags.Aperture && descriptor.Flags.CacheCoherent,
    ...
    input.PhysicalAdapterIndex = 0; adapter.Ready = false;
    ...
    check(VioGpuWddmGetNodeMetadata(&adapter, 0, &metadata) == STATUS_SUCCESS && metadata.EngineType == DXGK_ENGINE_TYPE_3D &&
          !metadata.GpuMmuSupported && !metadata.IoMmuSupported, "typed WDK callback reports actual physical node");
    check(VioGpuWddmGetNodeMetadata(&adapter, 0x10000, &metadata) == STATUS_INVALID_PARAMETER, "combined adapter ordinal validated");
    ...
    check(info.FlagsWddm2.Value == (0x8000U | 1U | 4U) && info.PhysicalAdapterIndex == 0 && info.hAllocation == &allocation,
