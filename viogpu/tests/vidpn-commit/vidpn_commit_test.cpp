...
struct D3DKMDT_VIDPN_SOURCE_MODE
{
    struct
    {
        struct
        {
            D3DDDIFORMAT PixelFormat = D3DDDIFMT_A8R8G8B8;
        } Graphics;
    } Format;
};
...
struct Peer
{
    ...
    NTSTATUS step(const char *name)
    {
        ++calls;
        return std::strcmp(fail, name) ? STATUS_SUCCESS : Refusal;
    }
    ...
    bool released() const
    {
        return !(sources || sourceModes || targets || targetModes || pathRefs);
    }
} peer;
...
struct DXGK_VIDPN_INTERFACE
{
    ...
    {
        if (peer.step("topology"))
        {
            return Refusal;
        }
        *out = 2;
        *table = &topologyInterface;
        return STATUS_SUCCESS;
    }
    NTSTATUS pfnAcquireSourceModeSet(uintptr_t,
                                     uint32_t source,
                                     uintptr_t *out,
...
struct VioGpuDod
{
    ...
    struct
    {
        NTSTATUS DxgkCbQueryVidPnInterface(uintptr_t, unsigned, const DXGK_VIDPN_INTERFACE **table) const
        {
            ...
            *table = &vidpnInterface;
            return STATUS_SUCCESS;
        }
    } m_DxgkInterface;
    void CountDisplayEvent(unsigned index)
    {
        ++counters[index];
    }
    void RecordDisplayValue(unsigned index, LONG value)
    {
        counters[index] = value;
    }
    NTSTATUS IsVidPnSourceModeFieldsValid(const D3DKMDT_VIDPN_SOURCE_MODE *)
    {
        return peer.step("sourceValidate");
    }
                                  ...
                                  const D3DKMDT_VIDEO_SIGNAL_INFO *signal)
    {
        ...
        ++peer.commits;
        peer.currentClock = signal->PixelRate;
        return STATUS_SUCCESS;
    }
...
int main()
{
    ...
    for (uint32_t source : {0u, D3DDDI_ID_ALL})
    {
        ...
        request.AffectedVidPnSourceId = source;
        ...
        check(device.counters[5] == 1 && device.counters[6] == STATUS_SUCCESS,
              "successful commit publishes real status");
    }
    for (const char *failure : {"query",
                                "topology",
                                "numPaths",
                                "sourceSet",
                                "sourcePinned",
    ...
    for (uint32_t invalid : {1u, 27u, D3DDDI_ID_UNINITIALIZED})
    {
        ...
        VioGpuDod device;
        DXGKARG_COMMITVIDPN request;
        request.AffectedVidPnSourceId = invalid;
        check(device.CommitVidPn(&request) == STATUS_GRAPHICS_INVALID_VIDEO_PRESENT_SOURCE && !peer.calls && device.counters[6] == STATUS_GRAPHICS_INVALID_VIDEO_PRESENT_SOURCE,
              "invalid source rejected before callbacks");
    }
    for (unsigned empty = 0; empty < 3; ++empty)
    {
        ...
        if (empty == 0)
        {
            request.Flags.PathPoweredOff = true;
        }
        if (empty == 1)
        {
            peer.paths = 0;
        ...
        if (empty == 2)
        {
            peer.pinSource = false;
        }
        check(device.CommitVidPn(&request) == STATUS_SUCCESS && peer.released() && !peer.commits && device.counters[6] == STATUS_SUCCESS,
              "powered-off/empty/unpinned transaction preserves selection");
    }
    peer = Peer{};
    peer.pinTarget = false;
    VioGpuDod device;
    DXGKARG_COMMITVIDPN request;
    check(device.CommitVidPn(&request) == STATUS_GRAPHICS_VIDPN_MODALITY_NOT_SUPPORTED && peer.released() && !peer.commits &&
                                                                                                              device.counters[6] == STATUS_GRAPHICS_VIDPN_MODALITY_NOT_SUPPORTED,
    ...
    {
        peer = Peer{};
        peer.source.Format.Graphics.PixelFormat = D3DDDIFMT_A8R8G8B8;
        VioGpuDod formatDevice;
        ...
        {
            check(status == STATUS_GRAPHICS_VIDPN_MODALITY_NOT_SUPPORTED && !peer.commits && peer.released() && peer.currentClock == 422060000 &&
                                                                                                                      formatDevice.counters[6] == STATUS_GRAPHICS_VIDPN_MODALITY_NOT_SUPPORTED,
    ...
    }
    {
        ...
        VioGpuDod formatDevice;
        DXGKARG_COMMITVIDPN formatRequest;
        check(formatDevice.CommitVidPn(&formatRequest,
                                       D3DDDIFMT_A8R8G8B8) == STATUS_GRAPHICS_VIDPN_MODALITY_NOT_SUPPORTED && !peer.commits &&
                                                                                                                  peer.released(),
              "unpinned source cannot satisfy a required output format");
    }
