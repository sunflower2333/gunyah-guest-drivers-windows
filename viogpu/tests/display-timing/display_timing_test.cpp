...
static void displayid(unsigned char *block,
                      unsigned width,
                      unsigned height,
                      unsigned hblank,
                      unsigned vblank,
...
int main(int argc, char **argv)
{
    ...
    const unsigned char dtd[] = {0x7c, 0x6b, 0, 0x18, 0x51, 0, 0x2d, 0x40, 0x30, 0x20, 0x35, 0, 0, 0, 0, 0, 0, 0x1e};
    std::memcpy(edid + 54, dtd, 18);
    edid[38] = 129;
    edid[39] = 0x80; // 1280x1024@60, same geometry, distinct rate
    edid[40] = 169;
    edid[41] = 0xc0; // 1600x900@60 (aspect ratio in TOP bits)
    edid[42] = 129;
    edid[43] = 0xa5; // 1280x1024@97
    checksum(edid);
    VIOGPU_DISPLAY_TIMING modes[64] = {};
    ...
    const unsigned char hostDtd[] = {0xa0,
                                     0xb4,
                                     0x80,
                                     0x30,
                                     0x72,
    ...
    checksum(edid);
    edid[71] |= 0x80;
    checksum(edid); // unsupported interlace
    ...
    edid[132] = 0x02;
    edid[133] = 0x3a; // 148.50MHz with same raster
    checksum(edid + 128);
    ...
    std::memcpy(malformed, extended, sizeof(extended));
    malformed[284] ^= 1;
    checksum(malformed + 256); // outerchecksumOK,innerbad
    assert(VioGpuReadEdidTimings(malformed, sizeof(malformed), modes, 64) == 5);
    std::memcpy(malformed, extended, sizeof(extended));
    malformed[267] |= 0x10;
    displayidChecksum(malformed + 256); // interlace
    assert(VioGpuReadEdidTimings(malformed, sizeof(malformed), modes, 64) == 5);
    std::memcpy(malformed, extended, sizeof(extended));
    malformed[263] = 19;
    displayidChecksum(malformed + 256); // partialtypeI
    assert(VioGpuReadEdidTimings(malformed, sizeof(malformed), modes, 64) == 5);
    ...
    oldHost[54] = 0x62;
    oldHost[55] = 0xc5; // installed505.30MHz wrap
    oldHost[126] = 0;
    checksum(oldHost);
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        const auto &t = modes[0];
        assert(VioGpuTimingRational(t.PixelClock,
                                    static_cast<unsigned long long>(t.TotalWidth) * t.TotalHeight,
                                    rn,
                                    rd));
