...
void RtlZeroMemory(void *destination, size_t size) { std::memset(destination, 0, size); }
void RtlCopyMemory(void *destination, const void *source, size_t size) { std::memcpy(destination, source, size); }
LONG InterlockedExchange(volatile LONG *target, LONG value) { LONG previous = *target; *target = value; return previous; }
LONG64 InterlockedExchange64(volatile LONG64 *target, LONG64 value) { LONG64 previous = *target; *target = value; return previous; }
LONG64 InterlockedCompareExchange64(volatile LONG64 *target, LONG64 value, LONG64 expected)
{ LONG64 previous = *target; if (previous == expected) *target = value; return previous; }
struct Rundown { bool allowed = true; unsigned acquired = 0; };
bool ExAcquireRundownProtection(Rundown *r) { if (!r->allowed) return false; r->acquired++; return true; }
void ExReleaseRundownProtection(Rundown *r) { r->acquired--; }
...
struct VioGpuDod
{
    ...
    bool IsHardwareResetRequested() const { return resetting; }
    void RecordNativeReadinessDiagnostic() { diagnostics++; }
...
int main()
{
    ...
    expected.Identity = { 0x44494c56, 1, 32, 1, identity.LowPart,
                          static_cast<uint32_t>(identity.HighPart), 1, 0 };
    ...
    for (UINT size = 0; size <= 176; size++)
    {
        ...
        for (size_t i = 0; i < data.size(); i++)
            if (i < 16 || i >= 16 + written) passed &= data[i] == 0xa5;
        check(passed, "bounded prefix or complete trailer with guards");
    }
    query.OutputDataSize = UINT32_MAX;
    data.fill(0xa5);
    check(QueryUmdPrivateInfo(&adapter, &query) == STATUS_SUCCESS &&
          std::memcmp(data.data() + 16, &expected, 160) == 0 && data[176] == 0xa5,
    ...
    auto rejects_without_writes = [&]() {
        ...
        NTSTATUS status = QueryUmdPrivateInfo(&adapter, &query);
        bool untouched = true;
        for (unsigned char c : data) untouched &= c == 0xa5;
    ...
    check(after_reset.AdapterInfo.ResetGeneration == 14 &&
          std::memcmp(&after_reset.Identity, &expected.Identity, 32) == 0,
