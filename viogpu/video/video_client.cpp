...
extern "C" HRESULT WINAPI VioGpuVideoOpen(UINT index, HANDLE *session, VGPU_VIDEO_OPEN *information)
{
    ...
    *session = nullptr;
    ...
    auto *detail = static_cast<SP_DEVICE_INTERFACE_DETAIL_DATA_W *>(HeapAlloc(GetProcessHeap(),
                                                                              HEAP_ZERO_MEMORY,
                                                                              bytes));
    if (!detail)
    {
    ...
    VGPU_VIDEO_HEADER request = {};
    Header(request, sizeof(request), 0);
    DWORD actual = 0;
    HRESULT hr = Call(c, IOCTL_VGPU_VIDEO_OPEN, &request, sizeof(request), information, sizeof(*information), actual);
    if (SUCCEEDED(hr) && (actual != sizeof(*information) || information->Header.Version != VGPU_VIDEO_VERSION ||
                          information->Header.Size != sizeof(*information) || !information->Header.Generation))
    {
    ...
    *session = reinterpret_cast<HANDLE>(c);
    return S_OK;
}
...
{
    if (!session || !buffer || buffer->BytesUsed > VGPU_VIDEO_MAX_BUFFER_BYTES || VmediaQueueIndex(buffer->Type) < 0 ||
        (buffer->Type == VMEDIA_CAPTURE && buffer->BytesUsed) || (buffer->BytesUsed && !data))
    {
        return E_INVALIDARG;
    }
    Client *c = reinterpret_cast<Client *>(session);
    ...
    *request = *buffer;
    Header(request->Header, sizeof(*request), c->Generation);
    if (buffer->BytesUsed)
    {
        CopyMemory(request + 1, data, buffer->BytesUsed);
    }
    DWORD actual = 0;
    HRESULT hr = Call(c, IOCTL_VGPU_VIDEO_QUEUE, request, size, nullptr, 0, actual);
...
{
    if (!session || !buffer || capacity > VGPU_VIDEO_MAX_BUFFER_BYTES || (capacity && !data))
    {
        return E_INVALIDARG;
    }
    Client *c = reinterpret_cast<Client *>(session);
    VGPU_VIDEO_BUFFER request = *buffer;
    Header(request.Header, sizeof(request), c->Generation);
    ...
    DWORD actual = 0;
    HRESULT hr = Call(c, IOCTL_VGPU_VIDEO_COPY, &request, sizeof(request), response, size, actual);
    if (SUCCEEDED(hr))
    {
        ...
        else
        {
            *buffer = *response;
            if (buffer->BytesUsed)
            {
                CopyMemory(data, response + 1, buffer->BytesUsed);
            }
        }
    }
