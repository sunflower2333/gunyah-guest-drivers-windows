...
{
enum class Result { Ok, NeedInput, NotAccepting, FormatChanged, Frame, Drained, Invalid, Unsupported, Failed };
struct Layout { uint32_t Width = 0, Height = 0, Stride = 0, Bytes = 0; };
struct Transport
{
...
inline bool Nv12Layout(const VMEDIA_FORMAT &format, Layout &layout)
{
    ...
    if (format.Type != VMEDIA_CAPTURE || VmediaRead32(format.Data + 8) != VMEDIA_NV12 ||
        !value.Width || !value.Height || (value.Width & 1) || (value.Height & 1) ||
        value.Stride < value.Width || (value.Stride & 1) ||
...
class DecoderSession
{
    ...
    Layout layout;
    Result Fail() { failed = true; return Result::Failed; }
    ...
    Result ConfigureCapture()
    {
        ...
        if (!transport.Control(4, &format) || !Nv12Layout(format, layout)) return Fail();
        ...
        if (!transport.Allocate(captures) || captures.Count < minimum[1] ||
            captures.Count > VGPU_VIDEO_MAX_BUFFERS || captures.BufferBytes < layout.Bytes)
            return Fail();
        for (uint32_t i = 0; i < captures.Count; ++i)
            if (!CaptureQueue(i)) return Fail();
        if (!transport.Stream(VMEDIA_CAPTURE, true)) return Fail();
        capture = true;
        changing = false;
        formatPending = true;
    ...
    Result FinishCapture()
    {
        ...
        framePending = false;
        frame = {};
        if (!isLast) return CaptureQueue(index) ? Result::Ok : Fail();
        if (changing)
        {
            VGPU_VIDEO_BUFFERS release = {};
            release.Type = VMEDIA_CAPTURE;
            if (!transport.Stream(VMEDIA_CAPTURE, false) || !transport.Allocate(release)) return Fail();
            capture = false;
            return ConfigureCapture();
        }
        if (!draining) return Fail();
        last = true;
        return Result::Ok;
    }
public:
    explicit DecoderSession(Transport &backend) : transport(backend) {}
    ~DecoderSession() { if (opened) (void)Close(); }
    DecoderSession(const DecoderSession &) = delete;
    DecoderSession &operator=(const DecoderSession &) = delete;
    const Layout &Format() const { return layout; }
    uint32_t InputBytes() const { return inputs.BufferBytes; }
    bool IsOpen() const { return opened; }
    bool HasFrame() const { return framePending; }
    bool CanInput() const
    {
        if (!opened || failed || draining || formatPending || framePending) return false;
        for (uint32_t i = 0; i < inputs.Count; ++i) if (!inputBusy[i]) return true;
        return false;
    }
    Result Open(uint32_t index, uint32_t width, uint32_t height)
    {
        ...
        if (!transport.Open(index)) return Result::Failed;
        opened = true;
        ...
        for (uint32_t i = 0; i < 64; ++i)
        {
            ...
            if (!transport.Control(2, format)) break;
            if (VmediaRead32(format + 44) == VMEDIA_H264) { h264 = true; break; }
        }
        if (!h264) { (void)Close(); return Result::Unsupported; }
        for (uint32_t event : {VMEDIA_EVENT_SOURCE_CHANGE, VMEDIA_EVENT_EOS})
        {
            uint8_t subscription[32] = {};
            VmediaWrite32(subscription, event);
            if (!transport.Control(90, subscription)) return Fail();
        }
        VMEDIA_FORMAT format = {};
        ...
        if (!transport.Allocate(inputs) || !inputs.Count || inputs.Count > VGPU_VIDEO_MAX_BUFFERS ||
            ...
            !transport.Stream(VMEDIA_OUTPUT, true)) return Fail();
        return Result::Ok;
    }
    Result Close()
    {
        ...
        inputs = {}; captures = {}; frame = {}; layout = {};
        return success ? Result::Ok : Result::Failed;
    }
    Result AcknowledgeFormat()
    {
        if (!formatPending || failed) return Result::Invalid;
        formatPending = false;
        return Result::Ok;
    }
    Result Submit(const void *bytes, uint32_t length, int64_t timestampUs)
    {
        ...
        if (!bytes || !length || length > inputs.BufferBytes) return Result::Invalid;
        if (!CanInput()) return Result::NotAccepting;
        for (uint32_t i = 0; i < inputs.Count; ++i)
        {
            if (inputBusy[i]) continue;
            VGPU_VIDEO_BUFFER buffer = {};
            buffer.Type = VMEDIA_OUTPUT;
            buffer.Index = i;
            buffer.BytesUsed = length;
            buffer.TimestampUs = timestampUs;
            if (!transport.Queue(buffer, bytes)) return Fail();
            inputBusy[i] = true;
            return Result::Ok;
        }
    ...
    Result Drain()
    {
        if (!opened || failed) return Result::Failed;
        if (draining) return Result::Ok;
        uint8_t command[72] = {};
        VmediaWrite32(command, 1); // DECODER_CMD_STOP: drain, not STREAMOFF.
        if (!transport.Control(96, command)) return Fail();
        draining = true;
        return Result::Ok;
    }
    Result Poll(uint32_t timeoutMs = 0)
    {
        if (!opened || failed || timeoutMs > 1000) return Result::Failed;
        if (formatPending) return Result::FormatChanged;
        if (framePending) return Result::Frame;
        bool busy = false;
        for (uint32_t i = 0; i < inputs.Count; ++i) busy |= inputBusy[i];
        if (last && !busy) return Result::Drained;
        ...
        for (uint32_t count = 0; count < 128; ++count)
        {
            ...
            if (!received) return Result::NeedInput;
            if (event.Event == VMEDIA_EVT_ERROR) return Fail();
            if (event.Event == VMEDIA_EVT_V4L2)
            {
                if (event.V4l2Type == VMEDIA_EVENT_SOURCE_CHANGE)
                {
                    if (!capture) return ConfigureCapture();
                    changing = true;
                }
                // EOS event is advisory: only LAST plus returned inputs ends drain.
                continue;
            }
            if (event.Event != VMEDIA_EVT_DQBUF) return Fail();
            ...
            if (event.Type != VMEDIA_CAPTURE || !capture || event.Index >= captures.Count ||
                ...
                (event.BytesUsed && event.BytesUsed < layout.Bytes)) return Fail();
            frame = event;
            framePending = true;
            if (event.BytesUsed) return Result::Frame;
    ...
    {
        if (failed) return Result::Failed;
        if (!framePending || !data || capacity < captures.BufferBytes) return Result::Invalid;
        VGPU_VIDEO_BUFFER buffer = {};
        buffer.Type = VMEDIA_CAPTURE;
        buffer.Index = frame.Index;
        if (!transport.Copy(buffer, data, capacity) || buffer.BytesUsed != frame.BytesUsed ||
            buffer.TimestampUs != frame.TimestampUs || buffer.Flags != frame.Flags) return Fail();
        timestampUs = buffer.TimestampUs;
        // A failed destination copy must not return the frame to the device.
        return Result::Ok;
    }
    Result ReleaseFrame() { return framePending && !failed ? FinishCapture() : Result::Invalid; }
    uint32_t CaptureCapacity() const { return captures.BufferBytes; }
};
} // namespace viogpu_video
#endif
