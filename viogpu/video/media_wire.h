...
#pragma pack(push, 8)
typedef struct VMEDIA_CMD { uint32_t Command, Padding; } VMEDIA_CMD;
typedef struct VMEDIA_IOCTL { VMEDIA_CMD Header; uint32_t SessionId, Code; } VMEDIA_IOCTL;
typedef struct VMEDIA_RESP { uint32_t Errno, Padding; } VMEDIA_RESP;
typedef struct VMEDIA_OPEN_RESP { VMEDIA_RESP Header; uint32_t SessionId, Padding; } VMEDIA_OPEN_RESP;
typedef struct VMEDIA_CONFIG { uint32_t DeviceCaps, DeviceType; uint8_t Card[32]; } VMEDIA_CONFIG;
typedef struct VMEDIA_SG { uint64_t Gpa; uint32_t Length, Padding; } VMEDIA_SG;
typedef struct VMEDIA_TIMEVAL { int64_t Seconds, Microseconds; } VMEDIA_TIMEVAL;
...
typedef struct VMEDIA_FORMAT { uint32_t Type, Padding; uint8_t Data[200]; } VMEDIA_FORMAT;
...
{
    VMEDIA_TIMEVAL t;
    t.Seconds = us / 1000000; t.Microseconds = us % 1000000;
    if (t.Microseconds < 0) { --t.Seconds; t.Microseconds += 1000000; }
    return t;
}
...
static inline int VmediaTimeToUs(VMEDIA_TIMEVAL t, int64_t *us)
{
    ...
    if (!us || t.Microseconds < 0 || t.Microseconds >= 1000000 ||
        t.Seconds > INT64_MAX / 1000000 || t.Seconds < minimumSeconds) return 0;
    if (t.Seconds == minimumSeconds) {
        if (t.Microseconds < minimumMicros) return 0;
        *us = INT64_MIN + (t.Microseconds - minimumMicros);
        return 1;
    }
    *us = t.Seconds * 1000000;
    if (*us > INT64_MAX - t.Microseconds) return 0;
    *us += t.Microseconds;
    return 1;
}
...
{
    switch (code) {
    case 2: return 64;                 /* ENUM_FMT */
    case 4: case 5: case 64: return 208; /* G/S/TRY_FMT */
    case 21: case 22: return 204;       /* G/S_PARM */
    case 27: case 28: return 8;         /* G/S_CTRL */
    case 36: return 68;                /* QUERYCTRL */
    case 74: return 44;                /* ENUM_FRAMESIZES */
    case 75: return 52;                /* ENUM_FRAMEINTERVALS */
    case 77: case 78: return 40;        /* ENCODER_CMD / TRY */
    case 90: case 91: return 32;        /* SUBSCRIBE / UNSUBSCRIBE_EVENT */
    case 94: case 95: return 64;        /* G/S_SELECTION */
    case 96: case 97: return 72;        /* DECODER_CMD / TRY */
    case 103: return 232;              /* QUERY_EXT_CTRL */
    default: return 0;
    }
}
...
{
    uint32_t size;
    if (!f || !bytes || VmediaQueueIndex(f->Type) < 0 || f->Data[180] != 1) return 0;
    size = VmediaRead32(f->Data + 20); /* pix_mp.plane_fmt[0].sizeimage */
    if (!size || size > limit) return 0;
    *bytes = size;
    return 1;
}
/* Build the actual QBUF packet, with exactly one driver-owned GPA run. */
static inline int VmediaBuildQbuf(
    uint32_t session, uint32_t type, uint32_t index, uint64_t gpa,
    uint32_t capacity, uint32_t used, int64_t pts, void *packet, size_t packetBytes)
{
    VMEDIA_IOCTL cmd;
    VMEDIA_BUFFER buffer;
    VMEDIA_PLANE plane;
    ...
    if (!out || packetBytes < required || VmediaQueueIndex(type) < 0 || !gpa ||
        !capacity || used > capacity || (type == VMEDIA_CAPTURE && used) ||
        gpa > UINT64_MAX - capacity) return 0;
    memset(&cmd, 0, sizeof(cmd)); memset(&buffer, 0, sizeof(buffer));
    memset(&plane, 0, sizeof(plane)); memset(&sg, 0, sizeof(sg));
    cmd.Header.Command=VMEDIA_CMD_IOCTL; cmd.SessionId=session; cmd.Code=15;
    buffer.Index=index; buffer.Type=type; buffer.Memory=VMEDIA_MEMORY_USERPTR;
    buffer.Field=1; buffer.Length=1; buffer.Timestamp=VmediaTimeFromUs(pts);
    buffer.Flags=VMEDIA_FLAG_TIMESTAMP_COPY;
    /* Cookie only: all actual addressing comes from the appended GPA SG. */
    plane.PointerOrOffset=1; plane.Length=capacity; plane.BytesUsed=used;
    sg.Gpa=gpa; sg.Length=capacity;
    memcpy(out,&cmd,sizeof(cmd)); out+=sizeof(cmd);
    memcpy(out,&buffer,sizeof(buffer)); out+=sizeof(buffer);
    memcpy(out,&plane,sizeof(plane)); out+=sizeof(plane);
