...
public:
    ...
    bool Control(uint32_t code, void *payload) override
    {
        ...
        if (FAILED(VioGpuVideoControl(handle, &control)) || control.LinuxErrno) return false;
...
HRESULT Status(Result result)
{
    switch (result)
    {
        ...
        case Result::Drained: return MF_E_TRANSFORM_NEED_MORE_INPUT;
        case Result::NotAccepting: return MF_E_NOTACCEPTING;
        case Result::FormatChanged: return MF_E_TRANSFORM_STREAM_CHANGE;
        case Result::Unsupported: return MF_E_INVALIDMEDIATYPE;
        case Result::Invalid: return E_INVALIDARG;
...
HRESULT CloneType(IMFMediaType *source, IMFMediaType **out)
{
    ...
    if (SUCCEEDED(hr)) hr = source->CopyAllItems(copy.Get());
    if (SUCCEEDED(hr)) *out = copy.Detach();
    return hr;
}
class Decoder final : public IMFTransform
{
    ...
    {
        if (!input) return MF_E_TRANSFORM_TYPE_NOT_SET;
        ...
        if (SUCCEEDED(hr)) hr = MFSetAttributeSize(type.Get(), MF_MT_FRAME_SIZE, w, h);
        if (SUCCEEDED(hr)) hr = type->SetUINT32(MF_MT_DEFAULT_STRIDE, w);
        if (SUCCEEDED(hr)) hr = type->SetUINT32(MF_MT_SAMPLE_SIZE, w * h * 3 / 2);
        ...
        if (SUCCEEDED(hr) && rateN && rateD) hr = MFSetAttributeRatio(type.Get(), MF_MT_FRAME_RATE, rateN, rateD);
        if (SUCCEEDED(hr)) *out = type.Detach();
        return hr;
    }
    HRESULT EnsureSession()
    {
        if (session.IsOpen()) return S_OK;
        Result result = session.Open(index, width, height);
        if (result != Result::Ok) { (void)session.Close(); return Status(result); }
    ...
    {
        if (session.IsOpen()) state = session.Poll(wait);
        return state;
    }
public:
    ...
    STDMETHODIMP QueryInterface(REFIID iid, void **object) override
    {
        ...
        *object = nullptr;
        if (iid != __uuidof(IUnknown) && iid != __uuidof(IMFTransform)) return E_NOINTERFACE;
    ...
    { if (!a || !b || !c || !d) return E_POINTER; *a = *b = *c = *d = 1; return S_OK; }
    STDMETHODIMP GetStreamCount(DWORD *a, DWORD *b) override
    { if (!a || !b) return E_POINTER; *a = *b = 1; return S_OK; }
    ...
    STDMETHODIMP GetOutputStreamInfo(DWORD id, MFT_OUTPUT_STREAM_INFO *info) override
    {
        ...
        *info = {};
        ...
        if (!output) return MF_E_TRANSFORM_TYPE_NOT_SET;
        return output->GetUINT32(MF_MT_SAMPLE_SIZE, &info->cbSize);
    }
    ...
    STDMETHODIMP GetInputAvailableType(DWORD id, DWORD typeIndex, IMFMediaType **type) override
    {
        ...
        *type = nullptr;
        if (typeIndex) return MF_E_NO_MORE_TYPES;
        ComPtr<IMFMediaType> value;
        HRESULT hr = MFCreateMediaType(&value);
        if (SUCCEEDED(hr)) hr = value->SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Video);
        if (SUCCEEDED(hr)) hr = value->SetGUID(MF_MT_SUBTYPE, MFVideoFormat_H264);
        if (SUCCEEDED(hr)) *type = value.Detach();
        return hr;
    }
    ...
    {
        if (id) return MF_E_INVALIDSTREAMNUMBER;
        if (flags & ~MFT_SET_TYPE_TEST_ONLY) return E_INVALIDARG;
        std::lock_guard<std::mutex> lock(mutex);
        if (session.IsOpen()) return MF_E_TRANSFORM_CANNOT_CHANGE_MEDIATYPE_WHILE_PROCESSING;
        if (!type) { if (!flags) { input.Reset(); output.Reset(); } return S_OK; }
        GUID major = {}, subtype = {};
        UINT32 w = 0, h = 0;
        if (FAILED(type->GetGUID(MF_MT_MAJOR_TYPE, &major)) || major != MFMediaType_Video ||
            ...
            FAILED(MFGetAttributeSize(type, MF_MT_FRAME_SIZE, &w, &h)) || !w || !h ||
            w > 8192 || h > 8192 || (w & 1) || (h & 1) || uint64_t(w) * h * 3 / 2 > VGPU_VIDEO_MAX_BUFFER_BYTES)
            return MF_E_INVALIDMEDIATYPE;
        UINT32 interlace = MFVideoInterlace_Progressive;
        (void)type->GetUINT32(MF_MT_INTERLACE_MODE, &interlace);
        if (interlace != MFVideoInterlace_Progressive) return MF_E_INVALIDMEDIATYPE;
        if (flags) return S_OK;
        ComPtr<IMFMediaType> copy;
        HRESULT hr = CloneType(type, &copy);
        if (FAILED(hr)) return hr;
        input = copy; output.Reset(); width = w; height = h;
        rateN = rateD = 0;
    ...
    STDMETHODIMP SetOutputType(DWORD id, IMFMediaType *type, DWORD flags) override
    {
        ...
        if (stride != w) return MF_E_INVALIDMEDIATYPE;
        if (flags) return S_OK;
        output = expected;
        if (state == Result::FormatChanged)
        {
            hr = Status(session.AcknowledgeFormat());
            if (SUCCEEDED(hr)) state = Result::NeedInput;
        }
        return hr;
    }
    ...
    STDMETHODIMP GetInputStatus(DWORD id, DWORD *flags) override
    {
        ...
        std::lock_guard<std::mutex> lock(mutex);
        *flags = input && output && !ended && (!session.IsOpen() || session.CanInput()) ? MFT_INPUT_STATUS_ACCEPT_DATA : 0;
        return S_OK;
    }
    ...
    STDMETHODIMP ProcessMessage(MFT_MESSAGE_TYPE message, ULONG_PTR) override
    {
        ...
        switch (message)
        {
            ...
            case MFT_MESSAGE_NOTIFY_END_STREAMING:
            {
                ...
                state = Result::NeedInput; sequencePending = true; ended = false;
            ...
            case MFT_MESSAGE_NOTIFY_START_OF_STREAM:
                if (ended && session.IsOpen()) return MF_E_INVALIDREQUEST; // flush/reopen ends old ownership.
                ended = false;
                return S_OK;
            case MFT_MESSAGE_NOTIFY_END_OF_STREAM: return S_OK;
            case MFT_MESSAGE_NOTIFY_BEGIN_STREAMING: return S_OK;
            case MFT_MESSAGE_SET_D3D_MANAGER: return E_NOTIMPL;
    ...
    STDMETHODIMP ProcessInput(DWORD id, IMFSample *sample, DWORD flags) override
    {
        ...
        std::lock_guard<std::mutex> lock(mutex);
        if (!input || !output) return MF_E_TRANSFORM_TYPE_NOT_SET;
        if (ended) return MF_E_NOTACCEPTING;
        HRESULT hr = EnsureSession();
        if (FAILED(hr)) return hr;
        Result current = Pump();
        if (current == Result::Failed) return E_FAIL;
        if (!session.CanInput()) return MF_E_NOTACCEPTING;
        ...
        if (FAILED(hr)) return hr;
        if (!AnnexB(data, bytes)) hr = MF_E_INVALID_STREAM_DATA;
        UINT32 headerBytes = 0;
        if (sequencePending) (void)input->GetBlobSize(MF_MT_MPEG_SEQUENCE_HEADER, &headerBytes);
        ...
        if (SUCCEEDED(hr) && headerBytes)
        {
            ...
            if (SUCCEEDED(hr)) hr = input->GetBlob(MF_MT_MPEG_SEQUENCE_HEADER, combined, headerBytes, nullptr);
            if (SUCCEEDED(hr) && !AnnexB(combined, headerBytes)) hr = MF_E_INVALIDMEDIATYPE;
            if (SUCCEEDED(hr)) memcpy(combined + headerBytes, data, bytes);
        }
        if (SUCCEEDED(hr)) hr = Status(session.Submit(combined ? combined : data, bytes + headerBytes, timestamp / 10));
        if (combined) HeapFree(GetProcessHeap(), 0, combined);
        HRESULT unlock = buffer->Unlock();
        if (SUCCEEDED(hr)) { sequencePending = false; hr = unlock; }
        return hr;
    }
    STDMETHODIMP ProcessOutput(DWORD flags, DWORD count, MFT_OUTPUT_DATA_BUFFER *buffers, DWORD *status) override
    {
        ...
        if (flags || count != 1 || buffers[0].dwStreamID || buffers[0].pSample) return E_INVALIDARG;
        std::lock_guard<std::mutex> lock(mutex);
        *status = 0; buffers[0].dwStatus = 0; buffers[0].pEvents = nullptr;
        if (!input || !output) return MF_E_TRANSFORM_TYPE_NOT_SET;
        if (!session.IsOpen()) return MF_E_TRANSFORM_NEED_MORE_INPUT;
        ...
        if (result != Result::Frame) return Status(result);
        const Layout layout = session.Format();
        const DWORD tightBytes = layout.Width * layout.Height * 3 / 2;
        ComPtr<IMFSample> sample;
        ComPtr<IMFMediaBuffer> buffer;
        HRESULT hr = MFCreateSample(&sample);
        if (SUCCEEDED(hr)) hr = MFCreateMemoryBuffer(tightBytes, &buffer);
        BYTE *scratch = nullptr, *destination = nullptr;
        if (SUCCEEDED(hr))
        {
            scratch = static_cast<BYTE *>(HeapAlloc(GetProcessHeap(), 0, session.CaptureCapacity()));
            if (!scratch) hr = E_OUTOFMEMORY;
        }
        int64_t timestamp = 0;
        if (SUCCEEDED(hr)) hr = Status(session.CopyFrame(scratch, session.CaptureCapacity(), timestamp));
        if (SUCCEEDED(hr) && (timestamp > INT64_MAX / 10 || timestamp < INT64_MIN / 10)) hr = MF_E_INVALID_TIMESTAMP;
        ...
        if (SUCCEEDED(hr)) hr = buffer->SetCurrentLength(tightBytes);
        if (SUCCEEDED(hr)) hr = sample->AddBuffer(buffer.Get());
        if (SUCCEEDED(hr)) hr = sample->SetSampleTime(timestamp * 10);
        if (SUCCEEDED(hr) && rateN && rateD) hr = sample->SetSampleDuration(LONGLONG(10000000ull * rateD / rateN));
        if (FAILED(hr)) return hr; // Retain CAPTURE so allocation failures are retryable.
        result = session.ReleaseFrame();
        if (result == Result::Failed) return E_FAIL;
        if (result == Result::FormatChanged) state = result;
