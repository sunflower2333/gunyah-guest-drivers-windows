...
static inline bool VioGpuTimingRational(unsigned long long numerator,
...
static inline bool VioGpuTimingValid(const VIOGPU_DISPLAY_TIMING &t)
{
    ...
    return t.Width >= 640 && t.Height >= 480 && t.Width <= 8192 && t.Height <= 8192 && t.TotalWidth > t.Width &&
           t.TotalHeight > t.Height && t.TotalWidth <= 16384 && t.TotalHeight <= 16384 && t.PixelClock >= total * 20 &&
           t.PixelClock <= total * 360 && VioGpuTimingRational(t.PixelClock, total, numerator, denominator) &&
...
static inline bool VioGpuAppendTiming(VIOGPU_DISPLAY_TIMING *modes,
                                      unsigned capacity,
                                      unsigned &count,
...
{
    if (extension[0] != 0x70 || extension[1] != 0x13 || extension[2] > 121 || !VioGpuEdidBlockValid(extension))
    {
        return false;
    }
...
static inline void VioGpuReadDisplayIdTimings(const unsigned char *edid,
                                              unsigned extensions,
                                              VIOGPU_DISPLAY_TIMING *modes,
                                              unsigned capacity,
                                              unsigned &count,
                                              bool preferred)
{
...
static inline unsigned VioGpuReadEdidTimings(const unsigned char *edid,
                                             unsigned bytes,
                                             VIOGPU_DISPLAY_TIMING *modes,
                                             unsigned capacity)
{
    ...
    struct Established
    {
        unsigned byte, bit, width, height, hz;
    };
    static const Established established[] = {{35, 5, 640, 480, 60},
                                              {35, 4, 640, 480, 67},
                                              {35, 3, 640, 480, 72},
                                              {35, 2, 640, 480, 75},
                                              {35, 1, 800, 600, 56},
    ...
    for (unsigned offset = 38; offset < 54; offset += 2)
    {
        ...
        {
            case 0:
                height = edid[19] < 3 ? width : width * 10 / 16;
                break;
            case 1:
                height = width * 3 / 4;
                break;
        ...
        VioGpuAppendTiming(modes, capacity, count, VioGpuVirtualTiming(width, height, 60U + (edid[offset + 1] & 63U)));
    }
