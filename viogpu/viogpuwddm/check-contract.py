...
def check_arm64_workflow_contract() -> None:
    ...
    product_version_fragments = (
        ...
        "[int]$env:DROIDVM_DRIVER_MINOR -ne 58602",
...
def check_native_driver_caps_contract() -> None:
    ...
    wddm_query = function_body("VioGpuWddmQueryAdapterInfo", WDDM_DDI_CODE)
    if re.search(r"\bdriverCaps\s*->", wddm_query):
        fail("the WDDM QueryAdapterInfo wrapper must not overwrite the versioned DriverCaps helper")
...
def check_callback_table() -> None:
    ...
    for forbidden in (
        ...
        if forbidden in body:
            fail(f"render-only registration must not expose display ownership callback {forbidden}")
...
def check_legacy_runtime_callback_contract() -> None:
    ...
    for fragment in (
        ...
        "VioGpuVsyncDelay100ns(now,m_CrtcNextDueTicks,frequency.QuadPart)",
        "ExSetTimer(timer,-delay,0,NULL)",
    ):
        if fragment not in rearm_vsync:
            fail(f"vblank rearm must use a gated, relative one-shot delay to the existing phase: {fragment}")
    if "ExAcquireFastMutex" in rearm_vsync or "m_CrtcVsyncTimer," in rearm_vsync:
        fail("callback rearm must not acquire the delete mutex or read a replaceable timer pointer")
...
def check_native_context_ownership() -> None:
    ...
    # Neither close failure is queue-wide.  SubmitNativeControl has already
    # confirmed the shared virtqueue transaction, and the reply is read from
    # this owner's own control window, so an unreadable reply or a nonzero Host
    # ret says nothing about shared request/response framing.  Poisoning there
    # was observed on device to block every later context teardown until the
    # device restarts, so the close path must fail only this context.
    close_guard_start = close_queue.find("if(!copied||response.ret!=0){")
    if close_guard_start < 0:
        fail("submitqueue CLOSE must treat an unreadable reply and a nonzero Host return alike")
    close_guard_end = close_queue.find("returnVioGpuHostContextUnknown;}", close_guard_start)
    if close_guard_end < 0:
        fail("submitqueue CLOSE must retain ownership on any incomplete response")
...
def check_wddm_private_abi(root: ET.Element) -> None:
    ...
    # the first publication skip its bind and paint onto the firmware's image.
...
def main() -> None:
    ...
    check_native_driver_caps_contract()
