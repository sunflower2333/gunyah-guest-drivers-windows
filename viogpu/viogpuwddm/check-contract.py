...
def check_pci_resource_lifetime() -> None:
    ...
    for fragment in (
        ...
        if map_host_visible.count(fragment) != 1:
            fail(f"host-visible blob slots must alias the validated whole BAR mapping: {fragment}")
    ...
    if "DxgkCbUnmapMemory" in unmap_host_visible or "GetMappedVA()" not in unmap_host_visible:
        fail("host-visible blob aliases must not independently unmap the whole BAR mapping")
