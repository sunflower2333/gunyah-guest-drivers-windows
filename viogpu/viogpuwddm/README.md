...
the adapter, and reproduces the black display after both a guarded hot load and
a clean boot. Its first submission-fault caller RVA is `0x160B0`; exact package
disassembly resolves the preceding call to the Present failure path in
`VioGpuWddmSubmitCommand`. The execution diagnostic remains `Open`, proving the
passive Present worker was never entered. The subsequent
`TransactionRegistration / STATUS_DEVICE_NOT_READY` rejection occurs after
reset has closed the publication gate and is not the original failure. The
current source extends the same first-fault transaction with the pre-worker
...
same record reports an already-requested reset, first-fault caller RVA
`0x160B0`, and an unclaimed execution diagnostic. The exact binary identifies
the direct caller as Present Submit's common pre-worker failure exit;
registration is expected to reject afterward. The current execution diagnostic
still classifies any later worker failure and snapshots its fence, resources,
placement offsets, and reset generations while allocation lifecycle locks
remain held. The new Submit-stage snapshot distinguishes resolution, contract,
prepatch, cancellation, work-reference, queue-transition, and passive-queue
rejection before reset. StartDevice
...
Present Submit accepts the forms emitted by the registered legacy runtime: a
zero-valued `DXGK_SUBMITCOMMANDFLAGS`, the explicit Present value (`Present=1`,
`Value=2`), and the redirected Present form (`Present=1`,
`RedirectedPresent=1`, `Value=6`). Paging, flip, null-rendering,
context-switch, and reserved flag combinations remain contract faults.
...
sizing/creation plus explicit zero-capability format and multisample queries.
It has no shader, draw, or submit entry points.
