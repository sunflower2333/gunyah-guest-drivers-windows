...
#include "../common/viogpu_native_surface_policy.h"
#include "../common/viogpu_native_ahb_access.h"
...
const UINT VIOGPU_WDDM_HOST_SURFACE_SEGMENT_ID = 2;
/* A policy budget also enforced by the host across live and deferred AHBs;
 * this is not a statement about physically dedicated VRAM. */
const ULONGLONG VIOGPU_WDDM_HOST_SURFACE_BUDGET = 256ULL * 1024 * 1024;
                                ...
                                _In_ const VIOGPU_WDDM_PAGING_DMA_PACKET *packet)
{
    ...
    if (privateData == NULL || packet == NULL || privateData->Signature != VIOGPU_WDDM_DMA_SIGNATURE ||
        ...
        ((packet->Flags & (VioGpuWddmPagingFlagPageIn | VioGpuWddmPagingFlagPageOut)) != 0 &&
         !transaction->TransferDataComplete && (packet->Flags & VioGpuWddmPagingFlagHostSurface) == 0) ||
    ...
    const UINT allowedFlags = VioGpuWddmPagingFlagPageIn | VioGpuWddmPagingFlagPageOut | VioGpuWddmPagingFlagFill |
                              ...
                              VioGpuWddmPagingFlagSoftwareCompleted | VioGpuWddmPagingFlagHostSurface;
    ...
    const BOOLEAN hostSurface = (packet->Flags & VioGpuWddmPagingFlagHostSurface) != 0;
...
BOOLEAN IsValidResourceSharePrivateData(const VOID *data, UINT size)
{
    ...
    return IsCurrentAbiHeader(&share.Header, sizeof(share)) && share.ShareKey != 0 && share.ShareKey <= MAXULONG &&
           share.Stride != 0 && (share.Flags & ~VIOGPU_WDDM_RESOURCE_SHARE_NATIVE_SURFACE) == 0 &&
           share.Reserved[0] == 0 && share.Reserved[1] == 0;
}

static BOOLEAN IsMatchingHostSurfaceOpen(_In_ const VIOGPU_WDDM_ALLOCATION *allocation,
                                          _In_ const DXGKARG_OPENALLOCATION *openAllocation)
{
    VIOGPU_WDDM_RESOURCE_SHARE share = {};
    if (openAllocation->PrivateDriverSize == sizeof(share))
    {
        __try
        {
            RtlCopyMemory(&share, openAllocation->pPrivateDriverData, sizeof(share));
        }
        __except (EXCEPTION_EXECUTE_HANDLER)
        {
            return FALSE;
    ...
    return allocation->HostSurface
               ? IsCurrentAbiHeader(&share.Header, sizeof(share)) &&
                     share.Flags == VIOGPU_WDDM_RESOURCE_SHARE_NATIVE_SURFACE &&
                     share.ShareKey == allocation->ShareKey && share.Stride == allocation->ShareStride &&
                     share.Reserved[0] == 0 && share.Reserved[1] == 0
                              ...
                              SIZE_T alignedSize)
{
    ...
    const UINT segmentId = allocation->HostSurface ? VIOGPU_WDDM_HOST_SURFACE_SEGMENT_ID : VIOGPU_WDDM_SEGMENT_ID;
    allocationInfo->PreferredSegment.SegmentId0 = segmentId;
    allocationInfo->PreferredSegment.Direction0 = 0;
    allocationInfo->SupportedReadSegmentSet = 1 << (segmentId - 1);
    allocationInfo->SupportedWriteSegmentSet = 1 << (segmentId - 1);
    allocationInfo->EvictionSegmentSet = 0;
    allocationInfo->PhysicalAdapterIndex = 0;
    allocationInfo->FlagsWddm2.Value = 0;
    BOOLEAN cpuVisible = !allocation->HostSurface && (allocation->Flags & VIOGPU_WDDM_ALLOCATION_CPU_VISIBLE) != 0;
...
VOID DestroyCreatedAllocations(DXGK_ALLOCATIONINFO *allocationInfo, UINT count)
{
    for (UINT index = 0; index < count; ++index)
    {
        ...
        if (allocation != NULL)
        {
            ReleaseHostSurfaceAllocation(allocation);
...
/* HostSurface residency has no guest CPU alias. Keep the mapping predicate
 * strict, so CPU copy/lock paths cannot accidentally accept these allocations. */
BOOLEAN HasResidentAllocationBacking(const VIOGPU_WDDM_ALLOCATION *allocation)
{
    return allocation != NULL && (allocation->HostSurface
        ? allocation->PlacementValid && VioGpuResourceNativeAhb(allocation->Resource2DState) &&
          allocation->Resource2DResetGeneration != 0 && allocation->Adapter->SupportsNativeAhbPaging()
        : HasMappedAllocationBacking(allocation));
}

UINT AllocationSegmentId(const VIOGPU_WDDM_ALLOCATION *allocation)
{
    return allocation->HostSurface ? VIOGPU_WDDM_HOST_SURFACE_SEGMENT_ID : VIOGPU_WDDM_SEGMENT_ID;
}
...
BOOLEAN IsStandardPresentSurface(const VIOGPU_WDDM_ALLOCATION *allocation)
{
    return allocation != NULL && (allocation->HostSurface || IsGdiSourceAllocation(allocation) ||
                                   ...
                                   const VIOGPU_NATIVE_CONTEXT_SNAPSHOT *snapshot)
{
    ...
    submission->HostCommandStreamSize = header->CommandStreamSize;
    ...
    InitializeListHead(&submission->ImportWaitLink);
    ...
    NTSTATUS importStatus = PinNativeSubmitImports(submission, header, allocationList, allocationListSize);
...
VOID NativeSubmissionComplete(_In_opt_ PVOID callbackContext)
{
    ...
    BOOLEAN valid = InterlockedCompareExchange(&submission->State,
                    ...
                    buffer->data_buf != NULL && buffer->data_size == submission->HostCommandStreamSize &&
                    ...
                    command->hdr.flags == expectedFlags && buffer->native_submit_host_fence != 0 &&
                    command->hdr.fence_id == buffer->native_submit_host_fence &&
                    ...
                    command->size == submission->HostCommandStreamSize && command->num_in_fences == 0 && response != NULL &&
                    response->type == VIRTIO_GPU_RESP_OK_NODATA && response->flags == expectedFlags &&
                    response->fence_id == buffer->native_submit_host_fence && response->ctx_id == submission->ContextId &&
    ...
    submission->ImportsGpuRetired = valid;
...
VOID QueueAdmittedNativeSubmission(VIOGPU_WDDM_SUBMISSION *submission)
{
    KeWaitForSingleObject(&submission->Context->ImportSubmitMutex, Executive, KernelMode, FALSE, NULL);
    VioGpuDod *adapter = submission->Adapter;
    UINT fenceId = static_cast<UINT>(submission->FenceId);
    UINT nodeOrdinal = submission->Context->NodeOrdinal;
    if (InterlockedCompareExchange(&submission->State, 0, 0) == VioGpuWddmSubmissionQuarantined)
    {
        KeReleaseMutex(&submission->Context->ImportSubmitMutex, FALSE);
        return;
    }
    BOOLEAN operationAcquired = adapter->AcquireNativeSubmissionOperation();
    NTSTATUS bindingStatus = operationAcquired && !submission->CancelRequested
                                 ? ValidateNativeRenderBindings(submission) : STATUS_DEVICE_NOT_READY;
    if (bindingStatus == STATUS_SUCCESS)
        bindingStatus = AdmitNativeSubmitImports(submission);
    if (bindingStatus == STATUS_PENDING)
    {
...
BOOLEAN EnsureStandard2DAllocationBacking(VIOGPU_WDDM_ALLOCATION *allocation)
{
    ...
    {
        return allocation->HostSurface ? HasResidentAllocationBacking(allocation) :
            allocation->PlacementValid && allocation->NativeAhbMapped && allocation->NativeAhbAddress != NULL;
    }
...
BOOLEAN ReconcileGdiSourcePlacementAfterReset(VIOGPU_WDDM_ALLOCATION *allocation)
{
    ...
    if ((!IsGdiSourceAllocation(allocation) && !IsStandardPrimaryAllocation(allocation) && !allocation->HostSurface) ||
        allocation->Adapter == NULL || !allocation->PlacementValid || !EnsureStandard2DAllocationBacking(allocation))
    {
        return FALSE;
    }

    return HasResidentAllocationBacking(allocation) && VioGpuResourceBackingAttached(allocation->Resource2DState) &&
                              ...
                              _In_ const VioGpuDod *adapter)
{
    return allocation != NULL && context != NULL && adapter != NULL &&
           ...
           (context->Type == VioGpuWddmContextGdi ||
            (allocation->HostSurface && context->Type == VioGpuWddmContextNative)) &&
           (IsGdiSourceAllocation(allocation) || IsStandardPrimaryAllocation(allocation) || allocation->HostSurface) &&
                                     ...
                                     _Out_ BOOLEAN *prepatched)
{
    ...
    *prepatched = FALSE;
    ...
    if (entry->SegmentId != AllocationSegmentId(allocation) || entry->PhysicalAddress.QuadPart < 0 ||
                                   ...
                                   VIOGPU_NATIVE_PRESENT_EXECUTION_DIAGNOSTIC *executionDiagnostic)
{
    ...
    *failureDetail = 0;
    ...
    if (source->HostSurface || destination->HostSurface)
    {
        /* A full-frame host surface can be published directly. Partial blits
         * and allocation-copy commands still require actual copy semantics. */
        *failureStage = VioGpuWddmPresentExecuteHostSurfaceProtocol;
        NTSTATUS nativeStatus = STATUS_INVALID_PARAMETER;
        if (source->HostSurface && !transaction->CopyOnly && transaction->RectCount == 1 &&
            transaction->SourceRect.left == 0 && transaction->SourceRect.top == 0 &&
            transaction->SourceRect.right == static_cast<LONG>(source->Width) &&
            transaction->SourceRect.bottom == static_cast<LONG>(source->Height) &&
            transaction->DestinationRect.left == 0 && transaction->DestinationRect.top == 0 &&
            transaction->DestinationRect.right == static_cast<LONG>(source->Width) &&
            transaction->DestinationRect.bottom == static_cast<LONG>(source->Height))
        {
            nativeStatus = PresentResidentHostSurface(transaction->Adapter, source,
                                                       transaction->SourcePlacementOffset, FALSE);
        }
        InitializePresentExecutionDiagnostic(transaction, *failureStage, nativeStatus,
    ...
    BOOLEAN gdiSource = (transaction->Context->Type == VioGpuWddmContextGdi || source->HostSurface) &&
                        IsStandardPresentSurface(source);
    if (NT_SUCCESS(status))
    {
        ...
        else if (NT_SUCCESS(status) && !HasResidentAllocationBacking(source))
        {
            status = STATUS_DEVICE_NOT_READY;
...
VOID InitializeSegmentDescriptor(Descriptor *descriptor, BOOLEAN hostSurface)
{
...
NTSTATUS QuerySegment(VioGpuDod *adapter, const DXGKARG_QUERYADAPTERINFO *queryAdapterInfo)
{
    ...
    if (segmentInfo->pSegmentDescriptor != NULL)
    {
        ...
        if (segmentInfo->NbSegment == 2)
            InitializeSegmentDescriptor(descriptor + 1, TRUE);
    }
 ...
 * all. These describe the same negotiated segment list as QuerySegment. */
...
static NTSTATUS QuerySegment4(VioGpuDod *adapter, const DXGKARG_QUERYADAPTERINFO *queryAdapterInfo)
{
    ...
    const UINT segmentCount = adapter->SupportsNativeAhbPaging() ? 2 : 1;
    /* On the count query the contract forbids touching any other member. */
    if (output->NbSegment == 0)
    {
        output->NbSegment = segmentCount;
        return STATUS_SUCCESS;
    }
    if (output->NbSegment != segmentCount || output->pSegmentDescriptor == NULL ||
    ...
    for (UINT index = 0; index < segmentCount; ++index)
    {
        ...
        InitializeSegmentDescriptor(&descriptor, index != 0);
        /* Preserve the OS descriptor stride and unknown future tail. */
        RtlCopyMemory(reinterpret_cast<BYTE *>(output->pSegmentDescriptor) +
                      index * output->SegmentDescriptorStride, &descriptor, sizeof(descriptor));
    }
...
struct VIOGPU_WDDM_NATIVE_SHARE_ENTRY
{
    ...
    SIZE_T PagingNextOffset;
    ULONGLONG RetiredProducerFence;
    ULONGLONG PresentCount;
...
static LIST_ENTRY g_VioGpuNativeAccessWaiters;
static LONG g_VioGpuNativeImportWorkers;
static KEVENT g_VioGpuNativeImportWorkersIdle;
...
static BOOLEAN AcquireNativeShareRegistry(_In_ BOOLEAN createIfMissing)
{
    ...
    for (;;)
    {
        ...
        if (state == VioGpuNativeShareRegistryNone)
        {
            ...
            KeInitializeSpinLock(&g_VioGpuNativeAccessLock);
            InitializeListHead(&g_VioGpuNativeAccessWaiters);
            KeInitializeEvent(&g_VioGpuNativeImportWorkersIdle, NotificationEvent, TRUE);
...
/* No registry mutex, control request slot or dispatcher is held while a
 * display release is pending. Wait-list references also cover callbacks that
 * run before QueueNativeAhbOperation returns. */
struct VIOGPU_NATIVE_IMPORT_WORK
{
    WORK_QUEUE_ITEM Work;
    VIOGPU_WDDM_SUBMISSION *Submission;
};

struct VIOGPU_NATIVE_HOST_PAGING_WORK
{
    WORK_QUEUE_ITEM Work;
...
static VOID RunNativeHostPagingWork(PVOID opaque)
{
    ...
    if (--g_VioGpuNativeImportWorkers == 0)
        KeSetEvent(&g_VioGpuNativeImportWorkersIdle, IO_NO_INCREMENT, FALSE);
...
static BOOLEAN DetachNativeHostPagingBatch(VIOGPU_WDDM_PAGING_PRIVATE *first)
{
    ...
    KIRQL irql;
    KeAcquireSpinLock(&g_VioGpuNativeAccessLock, &irql);
    ++g_VioGpuNativeImportWorkers;
    KeClearEvent(&g_VioGpuNativeImportWorkersIdle);
    KeReleaseSpinLock(&g_VioGpuNativeAccessLock, irql);
    first->Transaction.Adapter->ReleaseNativePassiveDispatch(&first->Work);
    ExInitializeWorkItem(&detached->Work, RunNativeHostPagingWork, detached);
    ExQueueWorkItem(&detached->Work, DelayedWorkQueue);
    return TRUE;
}

__declspec(code_seg(".text"))
static VOID ResumeNativeImportWork(PVOID opaque)
{
    auto work = static_cast<VIOGPU_NATIVE_IMPORT_WORK *>(opaque);
    auto submission = work->Submission;
    QueueAdmittedNativeSubmission(submission);
    DereferenceRenderSubmission(submission);
    delete work;
    KIRQL irql;
...
__declspec(code_seg(".text"))
static VOID WakeNativeImportWaitersLocked(VioGpuDod *adapter)
{
    PLIST_ENTRY link = g_VioGpuNativeAccessWaiters.Flink;
    while (link != &g_VioGpuNativeAccessWaiters)
    {
        auto submission = CONTAINING_RECORD(link, VIOGPU_WDDM_SUBMISSION, ImportWaitLink);
        link = link->Flink;
        if (adapter != NULL && submission->Adapter != adapter)
            continue;
        auto work = new (NonPagedPoolNx) VIOGPU_NATIVE_IMPORT_WORK;
        if (work == NULL)
        {
            /* Retain the wait-list reference. Reset/close retries the wake;
             * allocation failure can never fabricate GPU retirement. */
            InterlockedExchange(&submission->CancelRequested, TRUE);
            submission->Adapter->RequestHardwareResetAtAnyIrql();
            continue;
        }
        work->Submission = submission;
        RemoveEntryList(&submission->ImportWaitLink);
        InitializeListHead(&submission->ImportWaitLink);
        submission->ImportWaiting = FALSE;
        ++g_VioGpuNativeImportWorkers;
        KeClearEvent(&g_VioGpuNativeImportWorkersIdle);
        ExInitializeWorkItem(&work->Work, ResumeNativeImportWork, work);
                                    ...
                                    UINT resourceId, ULONGLONG sequence)
{
    ...
    KIRQL irql;
    KeAcquireSpinLock(&g_VioGpuNativeAccessLock, &irql);
    if (result == VioGpuHostContextConfirmed && resourceId == share->ResourceId &&
        sequence == share->Access.Sequence && share->Access.WaitPending)
    {
        share->Access.ReleasedSequence = sequence;
        ++share->ReleaseCount;
    }
    else
    {
...
static VIOGPU_WDDM_ALLOCATION *ResolveHostImportAllocation(const VIOGPU_WDDM_IMPORTED_REFERENCE &ref,
                                                          VIOGPU_WDDM_DEVICE *device,
                                                          const DXGK_ALLOCATIONLIST *list, UINT count,
                                                          BOOLEAN *prepatched)
{
    *prepatched = FALSE;
    if (ref.Reserved == 0 || ref.Reserved - 1 >= count)
        return NULL;
    const auto &item = list[ref.Reserved - 1];
    auto opened = reinterpret_cast<VIOGPU_WDDM_OPEN_ALLOCATION *>(item.hDeviceSpecificAllocation);
    auto allocation = opened == NULL ? NULL : opened->Allocation;
    if (opened == NULL || opened->Signature != VIOGPU_WDDM_OPEN_ALLOCATION_SIGNATURE || opened->Device != device ||
        !IsOwnedAllocation(allocation, device->Adapter) || !allocation->HostSurface || allocation->Destroying ||
        allocation->ShareKey != ref.ShareKey || allocation->PrivateData.Size != ref.Size ||
        allocation->Resource2DResetGeneration != ref.ResetGeneration || item.Reserved != 0 ||
        ((ref.Access & VIOGPU_WDDM_REFERENCE_WRITE) != 0) != (item.WriteOperation != 0) ||
        (opened->ReadOnly && (ref.Access & VIOGPU_WDDM_REFERENCE_WRITE) != 0) ||
        (item.SegmentId != 0 && item.SegmentId != VIOGPU_WDDM_HOST_SURFACE_SEGMENT_ID))
        return NULL;
    *prepatched = item.SegmentId == VIOGPU_WDDM_HOST_SURFACE_SEGMENT_ID &&
                  item.PhysicalAddress.QuadPart >= 0 && allocation->PlacementValid &&
                  static_cast<ULONGLONG>(item.PhysicalAddress.QuadPart) == allocation->PlacementOffset;
    return allocation;
}
                               ...
                               const DXGK_ALLOCATIONLIST *allocationList, UINT allocationListSize)
{
    ...
    if (header->Flags == VIOGPU_WDDM_RENDER_FLAGS_NONE)
        return STATUS_SUCCESS;
    const UINT count = header->Reserved[1];
    if (count == 0 || count > VIOGPU_WDDM_IMPORTED_REFERENCE_LIMIT)
        return STATUS_INVALID_PARAMETER;
    auto refs = reinterpret_cast<const VIOGPU_WDDM_IMPORTED_REFERENCE *>(
        reinterpret_cast<const BYTE *>(header) + header->Reserved[0]);
    submission->Imports = new (NonPagedPoolNx) VIOGPU_WDDM_SUBMISSION_IMPORT[count];
    if (submission->Imports == NULL)
        return STATUS_NO_MEMORY;
    RtlZeroMemory(submission->Imports, sizeof(*submission->Imports) * count);
    ...
    for (UINT i = 0; i < count; ++i)
    {
        ...
        if (ref.ShareKey == 0 || ref.Iova == 0 || ref.Size == 0 ||
            ref.ResetGeneration != submission->ResetGeneration || ref.Access == 0 ||
            (ref.Access & ~(VIOGPU_WDDM_REFERENCE_READ | VIOGPU_WDDM_REFERENCE_WRITE)) != 0)
        {
            status = STATUS_INVALID_PARAMETER;
            break;
        }
        for (UINT j = 0; j < i; ++j)
            if (refs[j].ShareKey == ref.ShareKey || refs[j].Iova == ref.Iova ||
                (ref.Reserved != 0 && refs[j].Reserved == ref.Reserved))
                status = STATUS_INVALID_PARAMETER;
        if (!NT_SUCCESS(status))
            break;
        ...
        for (PLIST_ENTRY link = g_VioGpuNativeImports.Flink; link != &g_VioGpuNativeImports; link = link->Flink)
        {
            ...
            if (candidate->Context == submission->Context && candidate->Key == ref.ShareKey &&
                candidate->Adapter == submission->Adapter && candidate->Iova == ref.Iova &&
                candidate->Size == ref.Size && candidate->ResetGeneration == ref.ResetGeneration)
            {
                entry = candidate;
                break;
            }
        ...
        const BOOLEAN ownerAlias = share != NULL && !share->HostSurface && share->OwnerAllocation != NULL &&
            ...
            share->OwnerAllocation->PrivateData.RequestedIova == ref.Iova &&
            share->OwnerAllocation->ContextResetGeneration == ref.ResetGeneration;
        if (share == NULL || (!ownerAlias && (entry == NULL || share->ResourceId != entry->ResourceId)) ||
            share->Size != ref.Size ||
            (share->HostSurface && share->SurfaceResetGeneration != ref.ResetGeneration))
        {
            status = STATUS_INVALID_HANDLE;
            break;
        }
        ...
        if (AcquireAllocationSubmissionReference(pinnedAllocation, submission->Adapter) != STATUS_SUCCESS)
        {
            status = STATUS_GRAPHICS_ALLOCATION_BUSY;
            break;
        }
...
VOID UnpinNativeSubmitImports(VIOGPU_WDDM_SUBMISSION *submission)
{
    for (UINT i = 0; i < submission->ImportCount; ++i)
    {
        ...
        if (entry != NULL)
            InterlockedDecrement(&entry->SubmissionReferences);
        InterlockedDecrement(&share->SubmissionReferences);
        if (submission->Imports[i].OwnerAllocation != NULL)
            ReleaseAllocationSubmissionReference(submission->Imports[i].OwnerAllocation);
    }
    delete[] submission->Imports;
    submission->Imports = NULL;
    submission->ImportCount = 0;
}
...
BOOLEAN RepackNativeSubmitImports(VIOGPU_WDDM_SUBMISSION *submission)
{
    ...
    if (submission->ImportCount == 0)
        return TRUE;
    const auto source = static_cast<const MSM_CCMD_GEM_SUBMIT_REQ *>(submission->CommandStream);
    const UINT ownedBytes = source->nr_bos * sizeof(VIOGPU_WDDM_MSM_SUBMIT_BO);
    const UINT commandBytes = source->nr_cmds * sizeof(VIOGPU_WDDM_MSM_SUBMIT_CMD);
    const UINT capacity = submission->CommandStreamSize + submission->ImportCount * sizeof(VIOGPU_WDDM_MSM_SUBMIT_BO);
    BYTE *storage = new (NonPagedPoolNx) BYTE[capacity];
    if (storage == NULL)
        return FALSE;
    auto packet = reinterpret_cast<MSM_CCMD_GEM_SUBMIT_REQ *>(storage);
    RtlCopyMemory(packet, source, sizeof(*source) + ownedBytes);
    auto bos = reinterpret_cast<VIOGPU_WDDM_MSM_SUBMIT_BO *>(packet->payload);
    for (UINT i = 0; i < submission->ImportCount; ++i)
    {
        const auto &ref = submission->Imports[i];
        auto share = static_cast<VIOGPU_WDDM_NATIVE_SHARE_ENTRY *>(ref.Share);
        UINT index = 0;
        while (index < packet->nr_bos && bos[index].Handle != share->ResourceId)
            ++index;
        if (index == packet->nr_bos)
        {
            ++packet->nr_bos;
            RtlZeroMemory(&bos[index], sizeof(bos[index]));
            bos[index].Handle = share->ResourceId;
            bos[index].Presumed = ref.Reference.Iova;
        }
        bos[index].Flags |= ref.Reference.Access;
    }
    const UINT newBosBytes = packet->nr_bos * sizeof(*bos);
    RtlCopyMemory(packet->payload + newBosBytes, source->payload + ownedBytes, commandBytes);
    packet->hdr.len = sizeof(*packet) + newBosBytes + commandBytes;
    const BOOLEAN refreshed = submission->Adapter->RefreshNativeSubmit(submission->VirtioBuffer, packet,
                                                                       packet->hdr.len, TRUE);
    if (refreshed)
        submission->HostCommandStreamSize = packet->hdr.len;
    delete[] storage;
    return refreshed;
}
...
NTSTATUS AdmitNativeSubmitImports(VIOGPU_WDDM_SUBMISSION *submission)
{
    ...
    if (InterlockedCompareExchange(&g_VioGpuNativeShareState, 0, 0) != VioGpuNativeShareRegistryReady)
        return STATUS_SUCCESS;
    VIOGPU_WDDM_NATIVE_SHARE_ENTRY *waits[VIOGPU_WDDM_IMPORTED_REFERENCE_LIMIT];
    UINT waitCount = 0;
    NTSTATUS status = STATUS_SUCCESS;
    KIRQL irql;
    KeAcquireSpinLock(&g_VioGpuNativeAccessLock, &irql);
    /* MSM/KGSL's inner user timestamp is context-local. Keep that submission
     * order even while another context can render and Present independently. */
    KeAcquireSpinLockAtDpcLevel(&submission->Context->SubmissionLock);
    for (PLIST_ENTRY link = submission->Context->PendingSubmissions.Flink;
         link != &submission->Context->PendingSubmissions; link = link->Flink)
    {
        auto entry = CONTAINING_RECORD(link, VIOGPU_WDDM_CONTEXT_SUBMISSION_ENTRY, Link);
        if (entry->Kind != VioGpuWddmContextSubmissionRender || entry->Owner == submission)
            continue;
        auto prior = static_cast<VIOGPU_WDDM_SUBMISSION *>(entry->Owner);
        if (prior->FenceId != 0 && static_cast<LONG>(prior->UmdFenceId - submission->UmdFenceId) < 0 &&
            !prior->ImportsHostIssued && prior->State != VioGpuWddmSubmissionQuarantined)
            status = STATUS_PENDING;
    }
    KeReleaseSpinLockFromDpcLevel(&submission->Context->SubmissionLock);
    if (InterlockedCompareExchange(&submission->CancelRequested, 0, 0) != 0 ||
        InterlockedCompareExchange(&submission->State, 0, 0) != VioGpuWddmSubmissionHostIssued)
        status = STATUS_DEVICE_NOT_READY;
    for (UINT i = 0; NT_SUCCESS(status) && i < submission->ImportCount; ++i)
    {
        auto share = static_cast<VIOGPU_WDDM_NATIVE_SHARE_ENTRY *>(submission->Imports[i].Share);
        const UINT access = submission->Imports[i].Reference.Access;
        auto allocation = submission->Imports[i].OwnerAllocation;
        if (share->Access.Poisoned || (share->HostSurface &&
            (!share->SurfaceResident || allocation == NULL || !allocation->PlacementValid ||
             allocation->Resource2DResetGeneration != submission->ResetGeneration)))
        {
            status = STATUS_DEVICE_NOT_READY;
            break;
        }
        ...
        if ((access & VIOGPU_WDDM_REFERENCE_WRITE) != 0 && share->HostSurface &&
            ...
            !share->Access.WaitPending)
        {
            share->Access.WaitPending = true;
            InterlockedIncrement(&share->AsyncReferences);
            waits[waitCount++] = share;
        }
    }
    if (status == STATUS_SUCCESS && !submission->Adapter->TryResumeNativePassiveDispatch(&submission->Work))
        status = STATUS_PENDING;
    if (status == STATUS_PENDING && !submission->ImportWaiting && ReferenceRenderSubmission(submission))
    {
        submission->ImportWaiting = TRUE;
        InterlockedExchange(&submission->Work.DisplayReleaseWait, TRUE);
        InsertTailList(&g_VioGpuNativeAccessWaiters, &submission->ImportWaitLink);
    }
    else if (status == STATUS_SUCCESS)
    {
        for (UINT i = 0; i < submission->ImportCount; ++i)
        {
            ...
            if ((submission->Imports[i].Reference.Access & VIOGPU_WDDM_REFERENCE_WRITE) != 0)
                ...
                ++share->Access.Readers;
            if (share->HostSurface)
                KeClearEvent(&share->ProducersIdle);
        }
        submission->ImportsAdmitted = TRUE;
        InterlockedExchange(&submission->Work.DisplayReleaseWait, FALSE);
    }
    KeReleaseSpinLock(&g_VioGpuNativeAccessLock, irql);
    for (UINT i = 0; i < waitCount; ++i)
    {
        auto share = waits[i];
        if (!share->Adapter->QueueNativeAhbOperation(share->ResourceId, share->SurfaceResetGeneration,
                                                    share->Access.Sequence, FALSE, NativeAhbReleaseObserved, share))
            NativeAhbReleaseObserved(share, VioGpuHostContextUnknown, share->ResourceId, share->Access.Sequence);
    }
    return status;
}

__declspec(code_seg(".text"))
VOID RetireNativeSubmitImports(VIOGPU_WDDM_SUBMISSION *submission, BOOLEAN confirmed)
{
    if (InterlockedCompareExchange(&g_VioGpuNativeShareState, 0, 0) != VioGpuNativeShareRegistryReady)
        return;
    ...
    if (submission->ImportWaiting)
    {
        ...
        InitializeListHead(&submission->ImportWaitLink);
        submission->ImportWaiting = FALSE;
        releaseWaitReference = TRUE;
    }
    if (submission->ImportsAdmitted)
    {
        for (UINT i = 0; i < submission->ImportCount; ++i)
        {
            auto share = static_cast<VIOGPU_WDDM_NATIVE_SHARE_ENTRY *>(submission->Imports[i].Share);
            ...
            if ((submission->Imports[i].Reference.Access & VIOGPU_WDDM_REFERENCE_WRITE) != 0)
                ...
                --share->Access.Readers;
            if (confirmed)
                share->RetiredProducerFence = submission->FenceId;
            if (share->HostSurface && !share->Access.Writer && share->Access.Readers == 0)
            {
    ...
    WakeNativeImportWaitersLocked(submission->Adapter);
    KeReleaseSpinLock(&g_VioGpuNativeAccessLock, irql);
    if (releaseWaitReference)
        DereferenceRenderSubmission(submission);
}

struct VIOGPU_NATIVE_AHB_PRESENT_COMPLETION
{
...
__declspec(code_seg(".text"))
static VOID NativeAhbPresentAccepted(PVOID opaque, VIOGPU_HOST_CONTEXT_RESULT result,
                                     UINT resourceId, ULONGLONG sequence)
{
    ...
    KIRQL irql;
    KeAcquireSpinLock(&g_VioGpuNativeAccessLock, &irql);
    const BOOLEAN accepted = result == VioGpuHostContextConfirmed && share->ResourceId == resourceId &&
        sequence != 0 && sequence != share->Access.Sequence && share->Access.PresentPending &&
    ...
    WakeNativeImportWaitersLocked(share->Adapter);
    KeReleaseSpinLock(&g_VioGpuNativeAccessLock, irql);
    /* At most 64 records per resource across its lifetime. Host diagnostics
     * map this resource/generation to the Android buffer identity. */
    if (accepted && (share->PresentCount <= 4 || (share->PresentCount & (share->PresentCount - 1)) == 0))
        DbgPrintEx(DPFLTR_IHVVIDEO_ID, DPFLTR_INFO_LEVEL,
            "VIOGPU AHB key=%I64x resource=%u generation=%I64u producer=%I64u sequence=%I64u presents=%I64u releases=%I64u directBranch=1\n",
            share->Key, resourceId, share->SurfaceResetGeneration, share->RetiredProducerFence,
            sequence, share->PresentCount, share->ReleaseCount);
    InterlockedDecrement(&share->AsyncReferences);
    KeSetEvent(&pending->Event, IO_NO_INCREMENT, FALSE);
...
{
    PAGED_CODE();
    auto allocation = transaction->Allocation;
    auto adapter = transaction->Adapter;
    const BOOLEAN pageIn = (transaction->Flags & VioGpuWddmPagingFlagPageIn) != 0;
    ...
    if (share == NULL || !share->HostSurface || share->ResourceId != transaction->ResourceId ||
        share->SurfaceResetGeneration != transaction->HostResetGeneration || share->AllocationReferences == 0 ||
        share->Size != allocation->PrivateData.Size || share->Size != allocation->BackingSize)
    {
        ReleaseNativeShareRegistry();
        return STATUS_DEVICE_NOT_READY;
    }
     ...
     * the passive dispatcher, registry/lifecycle mutex, transport slot or
     * hardware rundown. A front buffer has no arbitrary release deadline. */
    while (!admitted)
    {
        ...
        if (!NT_SUCCESS(status) || admitted)
            break;
        LARGE_INTEGER cancellationPoll;
        cancellationPoll.QuadPart = -100LL * 1000 * 10;
        KeWaitForSingleObject(&share->AccessChanged, Executive, KernelMode, FALSE, &cancellationPoll);
    }
    ...
    if (admitted)
    {
        ...
        lifecycle = status == STATUS_SUCCESS;
        const BOOLEAN transferStart = (transaction->Flags & VioGpuWddmPagingFlagTransferStart) != 0;
        if (status == STATUS_SUCCESS && (allocation->Resource2DResetGeneration != transaction->HostResetGeneration ||
            transaction->PlacementOffset > VIOGPU_WDDM_HOST_SURFACE_BUDGET ||
            allocation->BackingSize > VIOGPU_WDDM_HOST_SURFACE_BUDGET - transaction->PlacementOffset ||
            transaction->TransferOffset > allocation->BackingSize ||
            transaction->TransferSize > allocation->BackingSize - transaction->TransferOffset ||
            ((pageIn || pageOut) && transaction->TransferAddress == NULL) ||
        ...
        {
            const UINT direction = pageIn ? VIRTIO_GPU_NATIVE_AHB_PAGE_WRITE : VIRTIO_GPU_NATIVE_AHB_PAGE_READ;
            if (transferStart)
            {
    ...
    while (status == STATUS_SUCCESS && !bookkeepingOnly && completed < transaction->TransferSize)
    {
        ...
        const SIZE_T remaining = transaction->TransferSize - completed;
        const UINT length = static_cast<UINT>(remaining > VIRTIO_GPU_NATIVE_AHB_PAGING_MAX_BYTES
                                               ? VIRTIO_GPU_NATIVE_AHB_PAGING_MAX_BYTES : remaining);
        const UINT operation = fill ? VIRTIO_GPU_NATIVE_AHB_PAGE_FILL :
                               pageIn ? VIRTIO_GPU_NATIVE_AHB_PAGE_WRITE : VIRTIO_GPU_NATIVE_AHB_PAGE_READ;
        PVOID data = fill ? NULL : static_cast<BYTE *>(transaction->TransferAddress) + completed;
    ...
    if (admitted)
    {
        NTSTATUS lockStatus = AcquireAllocationLifecycle(allocation);
        if (lockStatus != STATUS_SUCCESS)
            status = lockStatus;
        KeAcquireSpinLock(&g_VioGpuNativeAccessLock, &irql);
        if (status == STATUS_SUCCESS && (pageIn || pageOut))
        {
            share->PagingNextOffset += completed;
            if ((transaction->Flags & VioGpuWddmPagingFlagTransferEnd) != 0)
            {
                ...
                else
                {
                    ...
                    share->SurfaceResident = pageIn;
                }
            }
        }
        else if (status == STATUS_SUCCESS)
            share->SurfaceResident = fill;
        if (status == STATUS_SUCCESS)
        {
            if (share->SurfaceResident)
                PublishStandardPlacement(allocation, transaction->PlacementOffset);
            else if (discard || (pageOut && (transaction->Flags & VioGpuWddmPagingFlagTransferEnd) != 0))
                ClearNativePlacement(allocation);
            transaction->TransferDataComplete = TRUE;
        }
        else
            share->Access.Poisoned = true;
        share->Access.Writer = false;
...
NTSTATUS PresentHostSurface(VioGpuDod *adapter, VIOGPU_WDDM_ALLOCATION *allocation)
{
    ...
    if (status != STATUS_SUCCESS || !idle ||
        ...
        status = STATUS_DEVICE_NOT_READY;
    auto pending = status == STATUS_SUCCESS ? new (NonPagedPoolNx) VIOGPU_NATIVE_AHB_PRESENT_COMPLETION : NULL;
    if (status == STATUS_SUCCESS && pending == NULL)
    ...
    if (status == STATUS_SUCCESS)
    {
        ...
        else
        {
             ...
             * may publish or observe release of a leased host surface. */
            adapter->LatchFlippedScanout(share->ResourceId, layout.Width, layout.Height);
                                    ...
                                    ULONGLONG expectedPlacement, BOOLEAN modeChange)
{
    ...
    if (status != STATUS_SUCCESS)
        return NT_SUCCESS(status) ? STATUS_DEVICE_NOT_READY : status;
    status = allocation->HostSurface && allocation->PlacementValid &&
             allocation->PlacementOffset == expectedPlacement && EnsureStandard2DAllocationBacking(allocation)
                 ? AcquireAllocationSubmissionReference(allocation, adapter) : STATUS_DEVICE_NOT_READY;
    /* A paging producer can already own its writer lease and need this mutex.
     * Pin the allocation, then drop the mutex before waiting ProducersIdle. */
    KeReleaseMutex(&allocation->LifecycleMutex, FALSE);
    if (status == STATUS_SUCCESS)
    {
        status = PresentHostSurface(adapter, allocation);
        if (status == STATUS_SUCCESS && modeChange)
            adapter->SetCrtcVsyncPrimaryAddress(expectedPlacement);
        ReleaseAllocationSubmissionReference(allocation);
    }
    return status;
}

/* Only the registry owns the host resource ID. A wrapper allocation and every
 * importer lease keep it alive after FREE. Host UNREF itself retains Android
 * readers until their release fences signal; refusal/timeout retains our
 * ledger as well, so no busy backing can be recycled. */
static VOID CollectNativeSurfacesLocked(_In_ VioGpuDod *adapter)
{
    PLIST_ENTRY link = g_VioGpuNativeShares.Flink;
    while (link != &g_VioGpuNativeShares)
    {
        VIOGPU_WDDM_NATIVE_SHARE_ENTRY *share = CONTAINING_RECORD(link, VIOGPU_WDDM_NATIVE_SHARE_ENTRY, Link);
        link = link->Flink;
        if (share->Adapter != adapter || !share->HostSurface ||
            InterlockedCompareExchange(&share->SubmissionReferences, 0, 0) != 0 ||
            InterlockedCompareExchange(&share->AsyncReferences, 0, 0) != 0 ||
            !VioGpuNativeSurfaceMayRetire(share->OwnerReleased != FALSE, share->ImportReferences,
                                         share->AllocationReferences))
        {
            continue;
        }
        BOOLEAN released = FALSE;
        if (adapter->Destroy2DResource(share->ResourceId, &share->SurfaceState,
                                       &share->SurfaceResetGeneration, &released, TRUE) != VioGpuHostContextConfirmed ||
            !released || !adapter->Release2DResourceId(share->ResourceId))
        {
            continue;
        }
        RemoveEntryList(&share->Link);
        ObDereferenceObject(share->OwnerProcess);
        delete share;
    }
}

static NTSTATUS HandleNativeSurfaceEscape(_In_ VioGpuDod *adapter, _In_ const DXGKARG_ESCAPE *escape)
{
    PAGED_CODE();
    ...
    if (!IsCurrentAbiHeader(&request.Header, sizeof(request)) || !VioGpuNativeSurfaceRequestValid(&request))
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (request.Opcode == VIOGPU_WDDM_ESCAPE_FREE_NATIVE_SURFACE)
    {
        ...
        if (share == NULL || !share->HostSurface || share->OwnerReleased ||
            share->OwnerProcess != PsGetCurrentProcess() || !VioGpuNativeSurfaceIdentityMatches(&request, &share->Surface))
        {
            status = STATUS_INVALID_HANDLE;
        }
        else
    ...
    else
    {
        ...
        for (PLIST_ENTRY link = g_VioGpuNativeShares.Flink; link != &g_VioGpuNativeShares; link = link->Flink)
        {
            VIOGPU_WDDM_NATIVE_SHARE_ENTRY *entry = CONTAINING_RECORD(link, VIOGPU_WDDM_NATIVE_SHARE_ENTRY, Link);
            if (entry->Adapter == adapter && entry->HostSurface)
            {
                ++count;
                bytes += entry->Size;
            }
        ...
        else if (count >= 64 || bytes >= VIOGPU_WDDM_HOST_SURFACE_BUDGET)
        {
            status = STATUS_INSUFFICIENT_RESOURCES;
        }
        else
        {
            VIOGPU_WDDM_NATIVE_SHARE_ENTRY *share = new (NonPagedPoolNx) VIOGPU_WDDM_NATIVE_SHARE_ENTRY;
            if (share == NULL)
            {
                status = STATUS_NO_MEMORY;
            }
            else
            {
                ...
                share->HostSurface = TRUE;
                KeInitializeEvent(&share->ProducersIdle, NotificationEvent, TRUE);
                KeInitializeEvent(&share->AccessChanged, NotificationEvent, FALSE);
                share->OwnerProcess = PsGetCurrentProcess();
                ObReferenceObject(share->OwnerProcess);
                share->Key = NewNativeShareKeyLocked(adapter);
                share->ResourceId = adapter->Allocate2DResourceId();
                UINT format = 0;
                VioGpuNativeAhbFourccToVirtioFormat(request.Fourcc, &format);
                VIOGPU_PRIMARY_SCANOUT_LAYOUT layout = {};
                VIOGPU_HOST_CONTEXT_RESULT result = share->ResourceId == 0 ? VioGpuHostContextNotSubmitted :
                    adapter->Create2DResourceBacking(share->ResourceId, format, request.Width, request.Height,
                                                     0, NULL, 0, &share->SurfaceState, &share->SurfaceResetGeneration,
                                                     FALSE, TRUE, &layout);
                if (share->ResourceId == 0)
                {
                    ObDereferenceObject(share->OwnerProcess);
                    delete share;
                    status = STATUS_INSUFFICIENT_RESOURCES;
                ...
                {
                    /* Publish failed/unknown creates too: their host ownership
                    ...
                    else
                    {
                        ...
                        request.Size = layout.BackingSize;
                        request.ResetGeneration = generation;
                        request.ResourceId = share->ResourceId;
                        request.Stride = layout.Stride;
                        request.PlaneCount = 1;
                        request.LayoutFlags = 1;
                                                ...
                                                _Out_ ULONGLONG *generation)
{
    ...
    *generation = 0;
    ...
    VIOGPU_WDDM_NATIVE_SHARE_ENTRY *share = FindNativeShareByKeyLocked(adapter, resource->ShareKey);
    UINT format = 0;
    NTSTATUS status = STATUS_INVALID_PARAMETER;
    if (share != NULL && share->HostSurface && !share->OwnerReleased && share->AllocationReferences == 0 &&
        share->OwnerProcess == PsGetCurrentProcess() &&
        share->SurfaceState == VioGpu2DResourceNativeAhbBackingAttached &&
        (info->Flags & (VIOGPU_WDDM_ALLOCATION_NATIVE | VIOGPU_WDDM_ALLOCATION_CPU_VISIBLE)) == 0 &&
        adapter->SupportsNativeAhbPaging() &&
        info->Size == share->Size && info->Width == share->Surface.Width && info->Height == share->Surface.Height &&
        info->Pitch == share->Surface.Stride && resource->Stride == share->Surface.Stride &&
        ResolveStandard2DFormat(FromPrivateFormat(info->Format), &format) &&
        VioGpuNativeAhbFormatMatches(share->Surface.Fourcc, format))
    {
        BOOLEAN retired = FALSE;
...
{
    BOOLEAN retired = FALSE;
    if (!allocation->Adapter->Reconcile2DResourceAfterReset(&allocation->Resource2DState,
                                                            &allocation->Resource2DResetGeneration, &retired))
    {
        return STATUS_DEVICE_NOT_READY;
    }
    if (!retired)
    {
        ...
        if (allocation->Adapter->Detach2DScanoutResource(allocation->ResourceId, &detached) != VioGpuHostContextConfirmed ||
            !detached || (allocation->NativeAhbMapped &&
                         allocation->Adapter->UnmapNativeAhbBlob(allocation->ResourceId) != VioGpuHostContextConfirmed))
        {
            return STATUS_DEVICE_NOT_READY;
        }
...
VOID RemoveNativeImportsForContext(_In_ VIOGPU_WDDM_CONTEXT *context)
{
    ...
    while (link != &g_VioGpuNativeImports)
    {
        ...
        {
            VIOGPU_WDDM_NATIVE_SHARE_ENTRY *share = FindNativeShareByKeyLocked(entry->Adapter, entry->Key);
            if (entry->HostSurface && share != NULL && share->ImportReferences != 0)
            {
                --share->ImportReferences;
            }
            RemoveEntryList(&entry->Link);
            CollectNativeSurfacesLocked(entry->Adapter);
            delete entry;
        }
    }
    ReleaseNativeShareRegistry();
...
static VOID FindNativeAllocationRangeByIova(_In_ VIOGPU_WDDM_CONTEXT *context,
                                            ...
                                            _Out_ VIOGPU_WDDM_ALLOCATION **allocation)
{
    *resourceId = 0;
    *length = 0;
    *allocation = NULL;
                                        ...
                                        _Out_ ULONG *stage)
{
    UINT resourceId = 0;
    ULONGLONG length = 0;
    VIOGPU_WDDM_ALLOCATION *ownerAllocation = NULL;
    FindNativeAllocationRangeByIova(context, request->Iova, snapshot->ContextId, &resourceId, &length, &ownerAllocation);
...
{
    VIOGPU_WDDM_NATIVE_SHARE_ENTRY *share = FindNativeShareByKeyLocked(adapter, request->ShareKey);
    if (share != NULL && share->HostSurface &&
        (share->SurfaceResetGeneration != snapshot->ResetGeneration ||
         share->Surface.ResetGeneration != snapshot->ResetGeneration ||
         (share->OwnerReleased && share->AllocationReferences == 0)))
    {
        *stage = 43;
        return STATUS_DEVICE_NOT_READY;
    }
    ...
    *resourceId = share != NULL ? share->ResourceId : 0;
    ...
    entry->HostSurface = share->HostSurface;
    ...
    *hostResult = static_cast<ULONG>(result);
    if (result != VioGpuHostContextConfirmed)
    {
        if (share->HostSurface && result != VioGpuHostContextNotSubmitted)
        {
            /* An uncertain attach/IOVA rollback still owns a lease until this
             * context is actually destroyed. It must block FREE's UNREF. */
            ++share->ImportReferences;
                                         ...
                                         _Out_ ULONG *stage)
{
    for (PLIST_ENTRY link = g_VioGpuNativeImports.Flink; link != &g_VioGpuNativeImports; link = link->Flink)
    {
        ...
        if (InterlockedCompareExchange(&entry->SubmissionReferences, 0, 0) != 0)
        {
            *stage = 44;
            return STATUS_GRAPHICS_ALLOCATION_BUSY;
        }
        if (adapter->AcquireNativeSubmissionOperation())
        {
            result = snapshot->Adapter->ReleaseNativeSharedResource(snapshot, entry->ResourceId, entry->HostSurface);
            adapter->ReleaseNativeSubmissionOperation();
        }
        if (!entry->HostSurface || result == VioGpuHostContextConfirmed)
        {
            VIOGPU_WDDM_NATIVE_SHARE_ENTRY *share = FindNativeShareByKeyLocked(adapter, entry->Key);
            if (entry->HostSurface && share != NULL && share->ImportReferences != 0)
            {
                --share->ImportReferences;
            }
                               ...
                               _Out_ BOOLEAN *fullyPrepatched)
{
    ...
    *fullyPrepatched = TRUE;
    ...
    if (header->Flags == VIOGPU_WDDM_RENDER_IMPORTED_REFERENCES)
    {
        const auto refs = reinterpret_cast<const VIOGPU_WDDM_IMPORTED_REFERENCE *>(
            reinterpret_cast<const BYTE *>(header) + header->Reserved[0]);
        for (UINT index = 0; index < header->Reserved[1]; ++index)
        {
            if (refs[index].Reserved == 0)
                continue;
            BOOLEAN resident = FALSE;
            if (ResolveHostImportAllocation(refs[index], device, allocationList, allocationListSize, &resident) == NULL)
                return STATUS_INVALID_HANDLE;
            if (!resident)
                *fullyPrepatched = FALSE;
        }
...
VOID VioGpuColorPresentWorker(_In_ PVOID context)
{
    ...
    if (status == STATUS_SUCCESS)
    {
        ...
        if (work->FenceEpoch == adapter->QueryNativeFenceEpoch() && !adapter->IsHardwareResetRequested() &&
            !allocation->HostSurface && IsStandardPrimaryAllocation(allocation) && allocation->PlacementValid &&
...
BOOLEAN VioGpuWddmDrainNativeImportWorkers(void)
{
    ...
    if (InterlockedCompareExchange(&g_VioGpuNativeShareState, 0, 0) != VioGpuNativeShareRegistryReady)
        return TRUE;
    LARGE_INTEGER timeout;
    timeout.QuadPart = -10LL * 1000 * 1000 * 10;
    return KeWaitForSingleObject(&g_VioGpuNativeImportWorkersIdle, Executive, KernelMode, FALSE, &timeout) == STATUS_SUCCESS;
}

/* Successful HWClose has retired host contexts/resources and drained callers.
 * Do not retain keys across a new adapter incarnation (its generation may
 * restart at one), nor retain process objects after device removal. */
VOID VioGpuWddmRetireNativeShares(_In_ VioGpuDod *adapter)
{
    ...
    link = g_VioGpuNativeShares.Flink;
    while (link != &g_VioGpuNativeShares)
    {
        VIOGPU_WDDM_NATIVE_SHARE_ENTRY *share = CONTAINING_RECORD(link, VIOGPU_WDDM_NATIVE_SHARE_ENTRY, Link);
        link = link->Flink;
        if (share->Adapter == adapter)
        {
            RemoveEntryList(&share->Link);
            if (share->OwnerProcess != NULL)
            {
                ObDereferenceObject(share->OwnerProcess);
            }
            delete share;
                                                                    ...
                                                                    DXGKARG_CREATEALLOCATION *createAllocation)
{
    ...
    const BOOLEAN hostSurface = resourceShare.Flags == VIOGPU_WDDM_RESOURCE_SHARE_NATIVE_SURFACE;
    if (hostSurface && createAllocation->NumAllocations != 1)
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    for (; createdCount < createAllocation->NumAllocations; ++createdCount)
    {
        ...
        else if ((privateData.Flags & VIOGPU_WDDM_ALLOCATION_NATIVE) != 0)
        {
            VIOGPU_NATIVE_CONTEXT_SNAPSHOT snapshot = {};
        ...
        if (allocation == NULL)
        {
            if (hostSurface)
            {
                VIOGPU_WDDM_ALLOCATION temporary = {};
                temporary.Adapter = adapter;
                temporary.ShareKey = resourceShare.ShareKey;
                temporary.ResourceId = standardResourceId;
                temporary.HostSurface = TRUE;
                ReleaseHostSurfaceAllocation(&temporary);
                standardResourceId = 0;
            }
        ...
        {
            allocation->Resource2DState = VioGpu2DResourceNativeAhbBackingAttached;
            allocation->Resource2DResetGeneration = surfaceGeneration;
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (status == STATUS_SUCCESS)
                    {
                        ...
                        else if (allocation->HostSurface)
                        {
                            status = UnmapHostSurfaceAllocation(allocation);
                        }
                                                                  ...
                                                                  CONST DXGKARG_OPENALLOCATION *openAllocation)
{
    ...
    for (; openedCount < openAllocation->NumAllocations; ++openedCount)
    {
        ...
        else if (!IsMatchingHostSurfaceOpen(allocation, openAllocation))
        {
            status = STATUS_GRAPHICS_DRIVER_MISMATCH;
        }
                                                                 ...
                                                                 DXGKARG_CREATECONTEXT *createContext)
{
    ...
    KeInitializeMutex(&context->ImportSubmitMutex, 0);
...
NTSTATUS ExecutePagingTransaction(_Inout_ VIOGPU_WDDM_PAGING_TRANSACTION *transaction,
                                  _Inout_ VIOGPU_NATIVE_PASSIVE_WORK *work)
{
    if (transaction == NULL || transaction->Signature != VIOGPU_WDDM_PAGING_TRANSACTION_SIGNATURE ||
    ...
    if (allocation->HostSurface)
        return ExecuteHostSurfacePaging(transaction, work);
    NTSTATUS status = AcquireAllocationLifecycle(allocation);
    if (status != STATUS_SUCCESS)
    {
        return status;
                               ...
                               _In_ UINT mdlOffset)
{
    ...
    if (nativeAhbPrimary)
    {
        ...
        VIOGPU_PRIMARY_SCANOUT_LAYOUT nativeLayout = {allocation->Width, allocation->Height, virtioFormat,
                                                       allocation->Pitch, allocation->BackingSize};
        VIOGPU_HOST_CONTEXT_RESULT result = allocation->HostSurface
                                               ? (allocation->Resource2DState == VioGpu2DResourceNativeAhbBackingAttached
                                                      ? VioGpuHostContextConfirmed : VioGpuHostContextNotSubmitted)
                                               : adapter->Create2DResourceBacking(allocation->ResourceId,
                                                                               virtioFormat,
                                                                               allocation->Width,
                                                                               allocation->Height,
                                                                               allocation->BackingSize,
        ...
        if (allocation->HostSurface)
        {
            NTSTATUS unmap = UnmapHostSurfaceAllocation(allocation);
            if (!NT_SUCCESS(unmap))
            {
                KeReleaseMutex(&allocation->LifecycleMutex, FALSE);
                return STATUS_GRAPHICS_ALLOCATION_BUSY;
        ...
        else if (VioGpuResourceNativeAhb(allocation->Resource2DState))
        {
            BOOLEAN released = FALSE;
                                 ...
                                 _In_ PHYSICAL_ADDRESS dummyPage)
{
    ...
    if ((IsStandardPrimaryAllocation(allocation) || allocation->HostSurface) &&
         ...
         allocation->NativeAhbAddress != NULL))
    {
        ...
        if (allocation->HostSurface)
        {
            NTSTATUS unmap = UnmapHostSurfaceAllocation(allocation);
            KeReleaseMutex(&allocation->LifecycleMutex, FALSE);
            return NT_SUCCESS(unmap) ? STATUS_SUCCESS : STATUS_GRAPHICS_ALLOCATION_BUSY;
        }
                                        ...
                                        _In_ UINT packetFlags)
{
    ...
    const BOOLEAN hostSurface = allocation->HostSurface;
    PVOID hostTransferAddress = NULL;
    if (status == STATUS_SUCCESS)
    {
    ...
    if (status == STATUS_SUCCESS && !hostSurface &&
        (allocation->Signature != VIOGPU_WDDM_ALLOCATION_SIGNATURE || allocation->Adapter != adapter ||
    ...
    BOOLEAN transferDataComplete = !hostSurface && (!transfer || nativeAhb);
    if (status == STATUS_SUCCESS && hostSurface)
    {
        packetFlags &= ~VioGpuWddmPagingFlagSoftwareCompleted;
        packetFlags |= VioGpuWddmPagingFlagHostSurface;
        if (!adapter->SupportsNativeAhbPaging() || allocation->Destroying ||
            placementOffset > VIOGPU_WDDM_HOST_SURFACE_BUDGET ||
            allocation->BackingSize > VIOGPU_WDDM_HOST_SURFACE_BUDGET - placementOffset ||
            transferOffset > allocation->BackingSize || transferSize > allocation->BackingSize - transferOffset)
            status = STATUS_INVALID_PARAMETER;
        else if (transfer)
            status = ResolveTransferMdlAddress(transferMdl, mdlOffset, transferSize, &hostTransferAddress);
        /* No data is touched until this packet owns a scheduled paging fence. */
    }
    else if (status == STATUS_SUCCESS && transfer && nativeAhb)
    {
    ...
    if (status == STATUS_SUCCESS)
    {
        ...
        transaction->TransferAddress = hostTransferAddress;
        transaction->HostResetGeneration = hostSurface ? allocation->Resource2DResetGeneration : 0;
                                                                     ...
                                                                     DXGKARG_BUILDPAGINGBUFFER *pagingBuffer)
{
    ...
    if (pagingBuffer->Operation == DXGK_OPERATION_MAP_APERTURE_SEGMENT)
    {
        ...
        if (allocation == NULL || allocation->Signature != VIOGPU_WDDM_ALLOCATION_SIGNATURE ||
            allocation->Adapter != adapter || allocation->HostSurface ||
    ...
    if (pagingBuffer->Operation == DXGK_OPERATION_UNMAP_APERTURE_SEGMENT)
    {
        ...
        if (allocation == NULL || allocation->Signature != VIOGPU_WDDM_ALLOCATION_SIGNATURE ||
            allocation->Adapter != adapter || allocation->HostSurface ||
            pagingBuffer->UnmapApertureSegment.SegmentId != VIOGPU_WDDM_SEGMENT_ID)
        {
            adapter->RequestHardwareResetAtAnyIrql();
    ...
    switch (pagingBuffer->Operation)
    {
        case DXGK_OPERATION_TRANSFER:
            ...
            if (pagingBuffer->Transfer.Source.SegmentId == 0 &&
                allocation != NULL && pagingBuffer->Transfer.Destination.SegmentId == AllocationSegmentId(allocation))
            {
            ...
            else if (allocation != NULL && pagingBuffer->Transfer.Source.SegmentId == AllocationSegmentId(allocation) &&
        ...
        case DXGK_OPERATION_FILL:
            ...
            if (allocation == NULL || pagingBuffer->Fill.Destination.SegmentId != AllocationSegmentId(allocation))
            {
                return CompletePagingBufferOperation(adapter, STATUS_INVALID_PARAMETER, VioGpuApertureStageSoftware);
            }
        ...
        case DXGK_OPERATION_DISCARD_CONTENT:
            ...
            if (allocation == NULL || pagingBuffer->DiscardContent.SegmentId != AllocationSegmentId(allocation))
            {
                return STATUS_SUCCESS;
            }
...
_Use_decl_annotations_ VOID NativePagingBatchWorker(PVOID callbackContext)
{
    ...
    if (!first->BatchDetached && first->BatchPrivateData != NULL &&
        first->BatchPrivateStart < first->BatchPrivateEnd && first->BatchPrivateEnd <= first->BatchPrivateDataSize &&
        (first->BatchPrivateEnd - first->BatchPrivateStart) % sizeof(VIOGPU_WDDM_PAGING_PRIVATE) == 0)
    {
        for (UINT offset = first->BatchPrivateStart; offset < first->BatchPrivateEnd;
             offset += sizeof(VIOGPU_WDDM_PAGING_PRIVATE))
        {
            auto item = reinterpret_cast<VIOGPU_WDDM_PAGING_PRIVATE *>(static_cast<BYTE *>(first->BatchPrivateData) + offset);
            if (item->Transaction.Allocation->HostSurface)
            {
                if (DetachNativeHostPagingBatch(first))
                    return;
                InterlockedExchange(&first->Transaction.CancelRequested, TRUE);
                break;
            }
        }
    }
    ...
    NTSTATUS status = STATUS_SUCCESS;
    UINT count = 0;
    /* Detached host paging acquires rundown only around each completed chunk;
     * it must not obstruct reset while waiting for a display release. */
    BOOLEAN operationAcquired = !first->BatchDetached && adapter != NULL && adapter->AcquireNativeSubmissionOperation();
    ...
    if (adapter == NULL || (!first->BatchDetached && !operationAcquired) || !rangeValid ||
    ...
    for (UINT index = 0; NT_SUCCESS(status) && index < count; ++index)
    {
        ...
        status = ExecutePagingTransaction(&pagingPrivate->Transaction, &first->Work);
    ...
    if (adapter != NULL)
    {
        if (!NT_SUCCESS(status))
            adapter->RequestHardwareResetAtAnyIrql();
        /* The DMA private memory can be reclaimed immediately when the fence
         * is published. Drop the passive-work link before that notification,
         * and never publish a successful fence for failed preservation. */
        adapter->CompleteNativePassiveWork(&first->Work);
        if (NT_SUCCESS(status) && !adapter->CompleteNativeSystemSubmission(fenceId, nodeOrdinal, engineOrdinal))
            adapter->RequestHardwareResetAtAnyIrql();
        if (operationAcquired)
        {
            adapter->ReleaseNativeSubmissionOperation();
...
_Use_decl_annotations_ NTSTATUS APIENTRY VioGpuWddmPatch(CONST HANDLE hAdapter, CONST DXGKARG_PATCH *patchArguments)
{
    ...
    if (candidatePrivate != NULL && candidatePrivate->Kind == VioGpuWddmDmaKindPresent)
    {
        ...
        if (status == STATUS_SUCCESS)
        {
            ...
            BOOLEAN valid = sourceValid && sourceOpen != NULL && destinationOpen != NULL &&
                            ...
                            IsStandardPresentSurface(destination) &&
                            HasResidentAllocationBacking(source) &&
                            EnsureStandard2DAllocationBacking(destination) &&
                            VioGpuResourceBackingAttached(destination->Resource2DState) &&
                            HasResidentAllocationBacking(destination) &&
                            ...
                            destinationPatch->SplitOffset == 0 && sourceEntry->SegmentId == AllocationSegmentId(source) &&
                            ...
                            destinationEntry->SegmentId == AllocationSegmentId(destination) &&
    ...
    if (NT_SUCCESS(status))
    {
        for (UINT index = 0; index < submission->ImportCount; ++index)
        {
            const auto &ref = submission->Imports[index].Reference;
            if (ref.Reserved == 0)
                continue;
            BOOLEAN resident = FALSE;
            auto allocation = ResolveHostImportAllocation(ref, submission->Context->Device,
                patchArguments->pAllocationList, patchArguments->AllocationListSize, &resident);
            if (allocation != submission->Imports[index].OwnerAllocation || !resident)
            {
                status = STATUS_DEVICE_NOT_READY;
                break;
            }
...
static NTSTATUS BuildAllocationBlit(CONST HANDLE hContext, DXGKARG_PRESENT *present, BOOLEAN copyOnly)
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        BOOLEAN gdiCandidate = (context->Type == VioGpuWddmContextGdi || source->HostSurface) &&
                               IsStandardPresentSurface(source);
        ...
        else if (sourcePrepatched && !HasResidentAllocationBacking(source))
        {
            reason = VioGpuWddmPresentDiagnosticSourcePlacement;
        }
        else if (destinationPrepatched && !HasResidentAllocationBacking(destination))
        {
...
static NTSTATUS QueueMmioFlip(_In_ VioGpuDod *adapter, _In_ CONST DXGKARG_SETVIDPNSOURCEADDRESS *setVidPnSourceAddress)
{
    ...
    target.ExpectedSegment = allocation == NULL ? VIOGPU_WDDM_SEGMENT_ID : AllocationSegmentId(allocation);
...
VioGpuWddmSetVidPnSourceAddress(CONST HANDLE hAdapter, CONST DXGKARG_SETVIDPNSOURCEADDRESS *setVidPnSourceAddress)
{
    ...
    else if (setVidPnSourceAddress->PrimarySegment != AllocationSegmentId(
                 reinterpret_cast<const VIOGPU_WDDM_ALLOCATION *>(setVidPnSourceAddress->hAllocation)))
    {
        status = STATUS_INVALID_PARAMETER;
    }
    else
