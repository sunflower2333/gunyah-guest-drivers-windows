...
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_3)
/* One unrotated, unscaled, opaque RGB plane on source 0 is the only overlay
 * configuration this adapter can honour: the Host scans out a single primary,
 * so anything that would need composition (a second plane, a stretch, a
 * rotation, per-pixel alpha) has to go back through DWM. Answering TRUE for
 * exactly that case is what lets dxgkrnl hand the fullscreen app's own
 * allocation straight to the scanout instead of copying it into DWM's primary
 * every frame -- see VioGpuWddmSetVidPnSourceAddressMpo3, which performs it.
 *
 * Declaring MaxOverlays without these two DDIs is what fails adapter start
 * with CM_PROB_FAILED_POST_START: dxgkrnl requires the whole set to be
 * coherent, so both are registered on the same gate as the caps. */
static BOOLEAN MpoPlaneIsDirectScanout(_In_ VioGpuDod *adapter,
                                       _In_ CONST DXGK_MULTIPLANE_OVERLAY_ATTRIBUTES3 *attributes,
                                       _In_ D3DDDI_VIDEO_PRESENT_SOURCE_ID sourceId,
                                       _In_ UINT layerIndex,
                                       _In_opt_ HANDLE hAllocation)
{
    if (adapter == NULL || attributes == NULL || sourceId != 0 || layerIndex != 0)
    {
        return FALSE;
    }
    /* No flip, no rotation, no blend: the Host has no compositor to do them. */
    if (attributes->Flags.Value != 0 || attributes->Rotation != D3DDDI_ROTATION_IDENTITY ||
        attributes->Blend.Value != 0)
    {
        return FALSE;
    }
    /* Source and destination must be the same extent -- a stretch would need a
     * scaler this scanout does not have. */
    const LONG srcWidth = attributes->SrcRect.right - attributes->SrcRect.left;
    const LONG srcHeight = attributes->SrcRect.bottom - attributes->SrcRect.top;
    const LONG dstWidth = attributes->DstRect.right - attributes->DstRect.left;
    const LONG dstHeight = attributes->DstRect.bottom - attributes->DstRect.top;
    if (srcWidth <= 0 || srcHeight <= 0 || srcWidth != dstWidth || srcHeight != dstHeight)
    {
        return FALSE;
    }
    /* The plane must be the whole target, at the origin: a partial overlay
     * would leave the rest of the screen undefined. */
    if (attributes->DstRect.left != 0 || attributes->DstRect.top != 0)
    {
        return FALSE;
    }
    const VIOGPU_WDDM_ALLOCATION *allocation = reinterpret_cast<const VIOGPU_WDDM_ALLOCATION *>(hAllocation);
    if (allocation == NULL || allocation->Signature != VIOGPU_WDDM_ALLOCATION_SIGNATURE ||
        allocation->Adapter != adapter || !IsStandardPrimaryAllocation(allocation))
    {
        return FALSE;
    }
    if (static_cast<LONG>(allocation->Width) != srcWidth || static_cast<LONG>(allocation->Height) != srcHeight)
    {
        return FALSE;
    }
    return TRUE;
...
VioGpuWddmCheckMultiPlaneOverlaySupport3(CONST HANDLE hAdapter, DXGKARG_CHECKMULTIPLANEOVERLAYSUPPORT3 *args)
{
    ...
    VioGpuDod *adapter = reinterpret_cast<VioGpuDod *>(hAdapter);
    adapter->CountDisplayEvent(VioGpuOverlayCheckQueries);
    args->Supported = FALSE;
    RtlZeroMemory(&args->ReturnInfo, sizeof(args->ReturnInfo));
    /* Post-composition is a second pass over the result; refuse it rather than
     * silently ignore it. */
    if (args->PlaneCount != 1 || args->ppPlanes == NULL || args->ppPlanes[0] == NULL ||
        args->PostCompositionCount != 0)
    {
        return STATUS_SUCCESS;
    }
    CONST DXGK_MULTIPLANE_OVERLAY_PLANE_WITH_SOURCE2 *plane = args->ppPlanes[0];
    if (MpoPlaneIsDirectScanout(adapter,
    ...
    {
        args->Supported = TRUE;
        adapter->CountDisplayEvent(VioGpuOverlayCheckAccepts);
    }
    return STATUS_SUCCESS;
}

_Use_decl_annotations_ NTSTATUS APIENTRY
VioGpuWddmSetVidPnSourceAddressMpo3(CONST HANDLE hAdapter,
                                    CONST DXGKARG_SETVIDPNSOURCEADDRESSWITHMULTIPLANEOVERLAY3 *args)
{
    if (hAdapter == NULL || args == NULL)
    {
        return STATUS_INVALID_PARAMETER;
    ...
    VioGpuDod *adapter = reinterpret_cast<VioGpuDod *>(hAdapter);
    adapter->CountDisplayEvent(VioGpuOverlayFlipCalls);
    if (args->VidPnSourceId != 0 || args->PlaneCount != 1 || args->ppPlanes == NULL ||
        args->ppPlanes[0] == NULL || args->pPostComposition != NULL)
    {
        adapter->CountDisplayEvent(VioGpuOverlayFlipRejects);
        return STATUS_INVALID_PARAMETER;
    }
    CONST DXGK_MULTIPLANE_OVERLAY_PLANE3 *plane = args->ppPlanes[0];
    if (plane->ContextCount == 0 || plane->ppContextData == NULL || plane->ppContextData[0] == NULL)
    {
        adapter->CountDisplayEvent(VioGpuOverlayFlipRejects);
        return STATUS_INVALID_PARAMETER;
    }
    CONST DXGK_PRIMARYCONTEXTDATA *context = plane->ppContextData[0];
    if (!MpoPlaneIsDirectScanout(adapter,
                                 &plane->PlaneAttributes,
                                 args->VidPnSourceId,
                                 plane->LayerIndex,
    ...
    /* Same target contract as QueueMmioFlip: the vsync reports this address and
     * the display worker binds it at PASSIVE_LEVEL. */
    const VIOGPU_WDDM_ALLOCATION *allocation =
            reinterpret_cast<const VIOGPU_WDDM_ALLOCATION *>(context->hAllocation);
    VioGpuFlipTarget target = {};
    target.SourceId = args->VidPnSourceId;
    target.Segment = context->SegmentId;
    target.ExpectedSegment = VIOGPU_WDDM_SEGMENT_ID;
    target.Address = context->SegmentAddress.QuadPart;
    target.HasAllocation = TRUE;
    target.OwnedByAdapter = TRUE;
    target.StandardPrimary = TRUE;
    target.PlacementValid = allocation->PlacementValid;
    target.PlacementOffset = allocation->PlacementOffset;
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_3)
    target.HighPrecision = IsHighPrecisionSurfaceFormat(allocation->Format);
    target.HighPrecisionAdmitted = adapter->IsNativeHdrModeAvailable();
#endif
    const VioGpuFlipTargetStatus verdict = VioGpuValidateFlipTarget(target);
    if (verdict != VioGpuFlipTargetAccepted)
    ...
    adapter->ClearColorPresentCompletion();
#endif
    adapter->PublishPendingFlip(context->hAllocation);
    adapter->CountDisplayEvent(VioGpuOverlayFlipAccepts);
    return STATUS_SUCCESS;
}
