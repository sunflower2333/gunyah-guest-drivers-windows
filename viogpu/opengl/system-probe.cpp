...
{
   if (argc != 3) return error("usage: system-probe --load-only|--system <absolute package opengl directory>");
   wchar_t path[32768];
   if (!GetFullPathNameW(argv[2], _countof(path), path, nullptr)) return error("package path");
   if (wcscat_s(path, L"\\")) return error("path length");
#if defined(_M_IX86)
   ...
   if (!wcscmp(argv[1], L"--load-only")) {
      if (wcscat_s(path, icd)) return error("ICD path length");
      HMODULE module = LoadLibraryExW(path, nullptr, LOAD_WITH_ALTERED_SEARCH_PATH);
      if (!module) return error("load system ICD proxy");
      ...
      if (!validate || !validate(1)) return error("real DrvValidateVersion");
      ...
      if (!runtime || !GetModuleFileNameW(runtime, actual, _countof(actual)) ||
          !GetSystemDirectoryW(expected, _countof(expected)) ||
          wcscat_s(expected, L"\\opengl32.dll") || _wcsicmp(actual, expected))
      ...
      wc.hInstance = GetModuleHandleW(nullptr); wc.lpszClassName = L"VioGpuSystemIcdProbe";
      if (!RegisterClassW(&wc)) return error("window class");
      HWND window = CreateWindowW(wc.lpszClassName, L"System ICD probe", WS_OVERLAPPEDWINDOW | WS_VISIBLE,
                                  0, 0, 160, 160, nullptr, nullptr, wc.hInstance, nullptr);
      HDC dc = GetDC(window);
      ...
      if (!index || !SetPixelFormat(dc, index, &format)) return error("system pixel format");
      HGLRC context = wglCreateContext(dc);
      if (!context || !wglMakeCurrent(dc, context)) return error("system WGL context");
      const char *renderer = reinterpret_cast<const char *>(glGetString(GL_RENDERER));
      std::printf("GL_RENDERER=%s\n", renderer ? renderer : "null");
      if (!renderer || !std::strstr(renderer, "zink") || !std::strstr(renderer, "TURNIP")) return error("accelerated renderer");
      glViewport(0, 0, 64, 64); glClearColor(0, 0, 1, 1); glClear(GL_COLOR_BUFFER_BIT);
      glColor3f(1, 0, 0); glBegin(GL_TRIANGLES);
      glVertex2f(-1,-1); glVertex2f(1,-1); glVertex2f(0,1); glEnd(); glFinish();
      GLubyte pixel[4] = {};
      glReadPixels(32,32,1,1,GL_RGBA,GL_UNSIGNED_BYTE,pixel);
      GLenum status = glGetError();
      std::printf("RGBA=%u,%u,%u,%u GL_ERROR=%u\n", pixel[0],pixel[1],pixel[2],pixel[3],status);
      if (status || pixel[0] < 240 || pixel[1] > 15 || pixel[2] > 15 || !SwapBuffers(dc)) return error("system raster/present");
   ...
   wchar_t actual[32768], expected[32768];
   if (!GetFullPathNameW(argv[2], _countof(expected), expected, nullptr) ||
       wcscat_s(expected, L"\\") || wcscat_s(expected, arch) ||
       wcscat_s(expected, L"\\libgallium_wgl.dll") || !gallium ||
