...
typedef struct VIDEO_EVENT_SLOT
{
    PVOID Address;
    PHYSICAL_ADDRESS Dma;
} VIDEO_EVENT_SLOT;
typedef struct VIDEO_EVENT_COPY
{
    ULONG Length;
    UCHAR Bytes[sizeof(VMEDIA_EVENT)];
} VIDEO_EVENT_COPY;
...
{
    LARGE_INTEGER value;
    value.QuadPart = -(LONGLONG)milliseconds * 10000;
    return value;
}
...
static int VideoPostEvent(VIDEO_DEVICE *d, VIDEO_EVENT_SLOT *slot)
{
    ...
    sg.physAddr = slot->Dma;
    sg.length = (ULONG)sizeof(VMEDIA_EVENT);
    ...
    do
    {
        while ((slot = (VIDEO_EVENT_SLOT *)virtqueue_get_buf(d->Queues[1], &length)) != NULL)
        {
            if (length >= 8 && length <= sizeof(VMEDIA_EVENT) && d->EventCount < VIDEO_EVENT_RING)
            {
                ...
                d->EventTail = (d->EventTail + 1) % VIDEO_EVENT_RING;
                ++d->EventCount;
            }
            else
            {
...
BOOLEAN VideoIsr(WDFINTERRUPT interrupt, ULONG messageId)
{
    ...
    if (!InterlockedCompareExchange(&d->Ready, 0, 0))
    {
        return FALSE;
    }
...
static NTSTATUS VideoCommand(VIDEO_DEVICE *d,
                             const void *request,
                             ULONG requestBytes,
                             void *response,
                             ULONG responseBytes,
                             ULONG *written)
{
    ...
    int result;
    *written = 0;
    if (!d->Ready || d->Poisoned)
    {
        return STATUS_DEVICE_NOT_READY;
    }
    if (requestBytes > VIDEO_DMA_BYTES || responseBytes > VIDEO_DMA_BYTES)
    {
        return STATUS_INVALID_BUFFER_SIZE;
    }
    RtlCopyMemory(d->Command, request, requestBytes);
    RtlZeroMemory(d->Response, VIDEO_DMA_BYTES);
    sg[0].physAddr = d->CommandDma;
    sg[0].length = requestBytes;
    sg[1].physAddr = d->ResponseDma;
    sg[1].length = responseBytes;
    WdfSpinLockAcquire(d->QueueLock);
    KeClearEvent(&d->CommandDone);
    d->UsedBytes = 0;
    d->CommandPending = TRUE;
    ...
    *written = d->UsedBytes;
    if (responseBytes && *written)
    {
        RtlCopyMemory(response, d->Response, *written);
    }
    return STATUS_SUCCESS;
}
/* Run one fixed-format ioctl; Linux errno stays distinct from NT transport status. */
static NTSTATUS VideoControl(VIDEO_DEVICE *d, ULONG code, void *payload, ULONG bytes, BOOLEAN hasReply, uint32_t *error)
{
    UCHAR request[sizeof(VMEDIA_IOCTL) + VGPU_VIDEO_CONTROL_BYTES];
    UCHAR response[sizeof(VMEDIA_RESP) + VGPU_VIDEO_CONTROL_BYTES];
    VMEDIA_IOCTL command;
    ULONG written;
    NTSTATUS status;
    RtlZeroMemory(&command, sizeof(command));
    command.Header.Command = VMEDIA_CMD_IOCTL;
    command.SessionId = d->SessionId;
    command.Code = code;
    if (bytes > VGPU_VIDEO_CONTROL_BYTES)
    {
        return STATUS_INVALID_BUFFER_SIZE;
    }
    ...
    *error = 0;
    status = VideoCommand(d,
                          request,
                          (ULONG)sizeof(command) + bytes,
                          response,
                          (ULONG)sizeof(VMEDIA_RESP) + (hasReply ? bytes : 0),
                          &written);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    if (written < sizeof(VMEDIA_RESP))
    {
        goto malformed;
    }
...
static void VideoPurgeEvents(VIDEO_DEVICE *d, int q)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        d->EventHead = (d->EventHead + 1) % VIDEO_EVENT_RING;
        --d->EventCount;
        if (q < 0 || (copy.Length >= sizeof(VMEDIA_BUFFER) + 8 && VmediaRead32(copy.Bytes) == VMEDIA_EVT_DQBUF &&
                      VmediaQueueIndex(VmediaRead32(copy.Bytes + 8 + 4)) == q))
        {
            continue;
        }
        d->Events[d->EventTail] = copy;
        d->EventTail = (d->EventTail + 1) % VIDEO_EVENT_RING;
        ++d->EventCount;
    }
    if (!d->EventCount)
    {
        KeClearEvent(&d->EventReady);
    }
    WdfSpinLockRelease(d->QueueLock);
}
...
static NTSTATUS VideoClose(VIDEO_DEVICE *d)
{
    ...
    if (d->Opened)
    {
        ...
        close.Header.Command = VMEDIA_CMD_CLOSE;
    ...
    {
        VideoFreeBuffers(d, 0);
        VideoFreeBuffers(d, 1);
...
static NTSTATUS VideoAllocate(VIDEO_DEVICE *d, VGPU_VIDEO_BUFFERS *a)
{
    ...
    ULONG i;
    int q = VmediaQueueIndex(a->Type);
    NTSTATUS status;
    PHYSICAL_ADDRESS low, high, boundary;
    if (q < 0 || a->Count > VGPU_VIDEO_MAX_BUFFERS || d->Streaming[q])
    {
        return STATUS_INVALID_PARAMETER;
    }
    if (a->Count && d->Count[q])
    {
        return STATUS_DEVICE_BUSY;
    }
    ...
    if (a->Count)
    {
        ...
        if (!VmediaFormatSize(&format, VGPU_VIDEO_MAX_BUFFER_BYTES, &size))
        {
    ...
    req.Count = a->Count;
    req.Type = a->Type;
    req.Memory = VMEDIA_MEMORY_USERPTR;
    status = VideoControl(d, 8, &req, sizeof(req), TRUE, &error);
    a->LinuxErrno = error;
    ...
    for (i = 0; i < req.Count; ++i)
    {
        ...
        if (d->AllocatedBytes + total > VGPU_VIDEO_MAX_TOTAL_BYTES)
        {
        ...
        b->AllocationBytes = total;
        d->AllocatedBytes += total;
        pa = (ULONGLONG)MmGetPhysicalAddress(b->Allocation).QuadPart;
        aligned = (pa + VIDEO_ALIGN - 1) & ~((ULONGLONG)VIDEO_ALIGN - 1);
        b->Address = (UCHAR *)b->Allocation + (SIZE_T)(aligned - pa);
        b->Gpa.QuadPart = (LONGLONG)aligned;
        b->Tracker.Capacity = size;
        b->Tracker.State = VgpuBufferIdle;
...
rollback:
    ...
    VideoFreeBuffers(d, q);
    req.Count = 0;
    (void)VideoControl(d, 8, &req, sizeof(req), TRUE, &error);
    return status;
}
...
static NTSTATUS VideoQueueBuffer(VIDEO_DEVICE *d, const VGPU_VIDEO_BUFFER *r, SIZE_T inputBytes)
{
    ...
    NTSTATUS status;
    VIDEO_BUFFER *b;
    if (q < 0 || r->Index >= d->Count[q] || r->Flags)
    {
        return STATUS_INVALID_PARAMETER;
    }
    b = &d->Buffers[q][r->Index];
    if (r->BytesUsed > b->Tracker.Capacity || (q == 1 && r->BytesUsed) ||
        inputBytes != sizeof(*r) + (q == 0 ? r->BytesUsed : 0))
    {
        return STATUS_INVALID_BUFFER_SIZE;
    }
    if (!VgpuVideoBeginQueue(&b->Tracker))
    {
        return STATUS_DEVICE_BUSY;
    }
    if (q == 0)
    {
        RtlCopyMemory(b->Address, r + 1, r->BytesUsed);
    }
    packetBytes = VmediaBuildQbuf(d->SessionId,
                                  r->Type,
                                  r->Index,
                                  (uint64_t)b->Gpa.QuadPart,
                                  b->Tracker.Capacity,
                                  r->BytesUsed,
                                  r->TimestampUs,
                                  packet,
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        if (status == STATUS_DEVICE_BUSY && !d->Poisoned)
        {
            b->Tracker.State = VgpuBufferIdle;
        }
        return status;
    ...
    {
        b->Tracker.State = VgpuBufferIdle;
        return error == 11 ? STATUS_DEVICE_BUSY : STATUS_UNSUCCESSFUL;
    }
...
{
    VIDEO_EVENT_COPY copy;
    VGPU_VIDEO_EVENT result;
    ...
    WdfSpinLockRelease(d->QueueLock);
    if (!VgpuVideoParseEvent(copy.Bytes, copy.Length, d->SessionId, &result, &offset))
    {
        goto malformed;
    }
    if (result.Event == VMEDIA_EVT_DQBUF)
    {
        ...
        if (q < 0 || result.Index >= d->Count[q] ||
            !VgpuVideoCompleteBuffer(&d->Buffers[q][result.Index].Tracker, &result, offset))
        {
            goto malformed;
        }
    }
    VideoHeader(&result.Header, sizeof(result), d->Generation);
    *e = result;
    if (result.Event == VMEDIA_EVT_ERROR)
    {
        InterlockedExchange(&d->EventFailed, 1);
    }
    return STATUS_SUCCESS;
...
static BOOLEAN VideoControlTypeValid(const VGPU_VIDEO_CONTROL *c)
{
    ...
    {
        case 2:
            return VmediaQueueIndex(VmediaRead32(c->Payload + 4)) >= 0;
        case 4:
        case 5:
        case 64:
        case 21:
...
void VideoIoctl(WDFQUEUE queue, WDFREQUEST request, size_t outputBytes, size_t inputBytes, ULONG code)
{
    ...
    switch (code)
    {
        case IOCTL_VGPU_VIDEO_OPEN:
            expected = sizeof(VGPU_VIDEO_HEADER);
            break;
        case IOCTL_VGPU_VIDEO_CONTROL:
            expected = sizeof(VGPU_VIDEO_CONTROL);
            break;
        case IOCTL_VGPU_VIDEO_BUFFERS:
            expected = sizeof(VGPU_VIDEO_BUFFERS);
            break;
        case IOCTL_VGPU_VIDEO_QUEUE:
        case IOCTL_VGPU_VIDEO_COPY:
            expected = sizeof(VGPU_VIDEO_BUFFER);
            break;
        case IOCTL_VGPU_VIDEO_EVENT:
            expected = sizeof(VGPU_VIDEO_EVENT);
            break;
        case IOCTL_VGPU_VIDEO_STREAM:
            expected = sizeof(VGPU_VIDEO_STREAM);
            break;
        case IOCTL_VGPU_VIDEO_CLOSE:
            expected = sizeof(VGPU_VIDEO_HEADER);
            break;
        default:
    ...
    if (code != IOCTL_VGPU_VIDEO_QUEUE && inputBytes != expected)
    {
    ...
    if (d->EventFailed && code != IOCTL_VGPU_VIDEO_CLOSE)
    {
        status = STATUS_DEVICE_PROTOCOL_ERROR;
        goto unlock;
    }
    ...
    {
        VMEDIA_CMD command = {VMEDIA_CMD_OPEN, 0};
        VMEDIA_OPEN_RESP response;
        VGPU_VIDEO_OPEN result;
        ULONG written;
        if (d->Opened)
        {
    ...
    else if (code == IOCTL_VGPU_VIDEO_CONTROL)
    {
        ...
        if (c.Reserved || !VmediaControlSize(c.Code) || c.PayloadBytes != VmediaControlSize(c.Code) ||
            !VideoControlTypeValid(&c))
        {
            status = STATUS_INVALID_PARAMETER;
            goto unlock;
        }
        ...
        status = VideoControl(d,
                              c.Code,
                              c.Payload,
                              c.PayloadBytes,
                              (BOOLEAN)VmediaControlHasReply(c.Code),
                              &c.LinuxErrno);
        if (NT_SUCCESS(status))
        {
            RtlCopyMemory(out, &c, sizeof(c));
            information = sizeof(c);
        }
    }
    else if (code == IOCTL_VGPU_VIDEO_BUFFERS)
    {
        ...
        a.LinuxErrno = 0;
        status = VideoAllocate(d, &a);
        if (NT_SUCCESS(status))
        {
            RtlCopyMemory(out, &a, sizeof(a));
            information = sizeof(a);
        }
    }
    ...
    else if (code == IOCTL_VGPU_VIDEO_EVENT)
    {
        ...
        if (NT_SUCCESS(status))
        {
            RtlCopyMemory(out, &event, sizeof(event));
            information = sizeof(event);
        }
    }
    else if (code == IOCTL_VGPU_VIDEO_COPY)
    {
        VGPU_VIDEO_BUFFER r = *(VGPU_VIDEO_BUFFER *)in;
        VIDEO_BUFFER *b;
        if (r.Type != VMEDIA_CAPTURE || r.Index >= d->Count[1])
        {
            status = STATUS_INVALID_PARAMETER;
            goto unlock;
        }
        b = &d->Buffers[1][r.Index];
        if (b->Tracker.State != VgpuBufferReturned)
        {
            status = STATUS_DEVICE_BUSY;
            goto unlock;
        }
        r.BytesUsed = b->Tracker.BytesUsed;
        r.Flags = b->Tracker.Flags;
        r.TimestampUs = b->Tracker.TimestampUs;
    ...
    {
        VGPU_VIDEO_STREAM s = *(VGPU_VIDEO_STREAM *)in;
        ULONG type = s.Type;
        int q = VmediaQueueIndex(type), i;
        if (q < 0 || s.On > 1 || s.Reserved)
        {
            status = STATUS_INVALID_PARAMETER;
            goto unlock;
        }
        ...
        if (NT_SUCCESS(status) && !s.LinuxErrno)
        {
            ...
            {
                VideoPurgeEvents(d, q);
                for (i = 0; i < (int)d->Count[q]; ++i)
                {
                    d->Buffers[q][i].Tracker.State = VgpuBufferIdle;
                }
            }
        }
        if (NT_SUCCESS(status))
        {
            RtlCopyMemory(out, &s, sizeof(s));
            information = sizeof(s);
        }
    }
    else
    {
...
static void VideoFreeDma(VIDEO_DEVICE *d)
{
    ...
    if (d->Command)
    {
        VirtIOWdfDeviceFreeDmaMemory(&d->Virtio.VIODevice, d->Command);
    }
    if (d->Response)
    {
        VirtIOWdfDeviceFreeDmaMemory(&d->Virtio.VIODevice, d->Response);
    }
    d->Command = NULL;
    d->Response = NULL;
    for (i = 0; i < VIDEO_EVENT_SLOTS; ++i)
    {
        if (d->EventSlots[i].Address)
        {
            VirtIOWdfDeviceFreeDmaMemory(&d->Virtio.VIODevice, d->EventSlots[i].Address);
        }
...
NTSTATUS VideoD0Entry(WDFDEVICE device, WDF_POWER_DEVICE_STATE previous)
{
    ...
    params[0].Interrupt = d->Interrupt;
    params[1].Interrupt = d->Interrupt;
...
NTSTATUS VideoAdd(WDFDRIVER driver, PWDFDEVICE_INIT init)
{
    ...
    power.EvtDevicePrepareHardware = VideoPrepare;
    power.EvtDeviceReleaseHardware = VideoRelease;
    power.EvtDeviceD0Entry = VideoD0Entry;
    power.EvtDeviceD0Exit = VideoD0Exit;
    ...
    KeInitializeEvent(&d->CommandDone, NotificationEvent, FALSE);
    KeInitializeEvent(&d->EventReady, NotificationEvent, FALSE);
    WDF_OBJECT_ATTRIBUTES_INIT(&attributes);
    attributes.ParentObject = device;
    status = WdfSpinLockCreate(&attributes, &d->QueueLock);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    status = WdfWaitLockCreate(&attributes, &d->SessionLock);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    WDF_INTERRUPT_CONFIG_INIT(&interrupt, VideoIsr, VideoDpc);
    interrupt.AutomaticSerialization = FALSE;
    status = WdfInterruptCreate(device, &interrupt, WDF_NO_OBJECT_ATTRIBUTES, &d->Interrupt);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    status = WdfDeviceCreateDeviceInterface(device, &VideoInterface, NULL);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    WDF_IO_QUEUE_CONFIG_INIT_DEFAULT_QUEUE(&queue, WdfIoQueueDispatchSequential);
    queue.EvtIoDeviceControl = VideoIoctl;
