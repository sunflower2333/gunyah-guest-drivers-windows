    ...
    _Inout_ PVIOSND_DEVICE Device)
{
     ...
     * revision of the spec cannot grow into it. */
    RtlZeroMemory(&Device->VendorConfig, sizeof(Device->VendorConfig));
    /*
     * Ask the transport how much device config there is before reading any of it. A stock
     * virtio-snd stops at the spec's 16 bytes, and virtio_get_config() answers a read past that
     * by leaving the buffer alone -- correct, and enough on its own for the magic check below to
     * fall through to the defaults -- but it announces each one with an ASSERT, so on a checked
     * build the probe breaks into the debugger once per byte for a device that is merely not
     * ours. Legacy transports report no length at all, which is the same answer: no block.
     */
    if (Device->Vdev.config_len < VIOSND_VENDOR_CFG_OFFSET + VIOSND_VENDOR_CFG_V1_SIZE) {
        VIOSND_LOG(DPFLTR_IHVDRIVER_ID,
                   DPFLTR_ERROR_LEVEL,
                   "viosnd: device config is %Iu bytes, no room for a vendor block; "
                   "using defaults\n",
                   Device->Vdev.config_len);
    } else {
        /* Read the version-1 prefix first. Reading the whole struct up front would, on a host
         * that publishes only version 1, pull in whatever lies past its config length and then
         * decide what to believe about it -- so ask for the part every version has, and only
         * then for the part this one says it has. */
        ...
        if (Device->VendorConfig.magic != VIOSND_VENDOR_CFG_MAGIC ||
            ...
            /* A version this driver does not have room for is not a reason to discard the
             * fields it does: keep the v1 prefix and leave the rest zero, which is what every
             * consumer already reads as "the host said nothing". */
            if (Device->VendorConfig.version >= 2u &&
                Device->Vdev.config_len >=
                    VIOSND_VENDOR_CFG_OFFSET + sizeof(VIOSND_VENDOR_CONFIG)) {
                virtio_get_config(&Device->Vdev,
            ...
            } else if (Device->VendorConfig.version >= 2u) {
                VIOSND_LOG(DPFLTR_IHVDRIVER_ID,
                           DPFLTR_ERROR_LEVEL,
                           "viosnd: vendor config says v%u but the device publishes only %Iu "
                           "bytes; keeping the v1 fields\n",
                           Device->VendorConfig.version,
                           Device->Vdev.config_len);
            }
            VIOSND_LOG(DPFLTR_IHVDRIVER_ID,
